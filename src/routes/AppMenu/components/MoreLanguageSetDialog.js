...
class MoreLanguageSetDialog extends Component{
	...
	componentDidMount(){
		apiGet(this.props.apiHost,'/contentI18N/getOne',{objectId:this.props.menusetView.id,object:this.props.object},
			(response)=>{
				this.setState({
					data:response.data||{},
		...
		return (
			<div>
				...
				<div className={'row first'}>
						<label className={'col-xs-4'}>{i18n.t(100235/*语言*/)}</label>
						<label className={'col-xs-4'}>{i18n.t(200749/*值*/)}</label>
				</div>
