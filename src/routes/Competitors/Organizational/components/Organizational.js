...
export class CompetitorOrganizational extends Component {
    constructor(props) {
        super(props);
        ...
        if (data == i18n.t(100437/*删除*/)) {
            ...
            Confirm("删除节点后将无法恢复,您确定要删除吗?", {
                    ...
                    apiForm(API_FOODING_DS, '/enterpriseOrg/delete', {id: rightObj.orgCompanyId}, (response) => {
                        ...
                        $("#CompetitorsjOrgChart" + this.state.id).html("");
                        that.getChild();
                    }, (error) => {
    ...
    getChild = () => {
        ...
        apiGet(API_FOODING_DS, '/enterpriseOrg/getTreeById', {
            ...
            that.setState({treeData: response.data}, () => {
                ...
                showlist.attr('id', 'org4' + this.state.id)
                this.showall(response.data, showlist);
                $("#CompetitorsjOrgChart" + this.state.id).append(showlist);
                $("#org4" + this.state.id).jOrgChart({
                    chartElement: '#CompetitorsjOrgChart' + this.state.id,//指定在某个dom生成jorgchart
    ...
    getXY = (obj, type) => {
        if (type === 'onclick') {//点击
            ...
            navAddTab({
                id: 'forwader-detail',
                name: I18n.t(100449/*竞争对手*/) + I18n.t(100097/*详情*/),
                component: I18n.t(100449/*竞争对手*/) + I18n.t(100097/*详情*/),
                url: '/competitors/detail'
            });
            this.props.router.push({pathname: '/competitors/detail', query: {id: obj.data.orgCompanyId}});
            ...
            this.setState({
                ...
                document.getElementById('CompetitorOrganizational' + this.state.id).focus();
            })
        }
    };
    ...
    render() {
        ...
        return (<div className={'client-body scroll'}
            ...
            <ul id={'CompetitorsjOrgChart' + this.state.id}>

            </ul>
            {rightFuncShow ?
                <RightFuncKeys iconArray={iconArray} x={x} y={y} onBlur={this.onBlur} handClick={this.handClick}
                               id={"CompetitorOrganizational" + this.state.id}/> : null}
...
export default NavConnect(CompetitorOrganizational);
