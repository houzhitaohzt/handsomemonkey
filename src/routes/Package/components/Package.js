...
import {apiGet,apiPost,apiForm,API_FOODING_ES,API_FOODING_DS,language,pageSize,sizeList} from '../../../services/apiCall';
...
class Package extends Component{
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100026/*包装新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/packaging/getOne',{id:record.record.id},(response)=>{
			that.setState({
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/packaging/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/packaging/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	onRowDoubleClick(record,index,checked){
		var that = this;
		apiGet(API_FOODING_DS,'/packaging/getOne',{id:record.id},(response)=>{
			that.setState({
				checkedData:response.data,
				rodalShow : true,
				showHeader:true,
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/packaging/getPage',object,
				(response)=>{	
					that.setState({	
...
export default NavConnect(Package);
