...
import SalvrtypePlug from './SalvrtypePlug';
...
class Salvrtype extends Component{
	constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
			...
			showSaveAdd:true,
			showSaveClose:true,
			title:I18n.t(100163/*新增托盘*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/salvrType/getOne',{id:record.record.id},(response)=>{
			that.setState({
	...
	onRowDoubleClick(record,index,checked){
		var that = this;
		apiGet(API_FOODING_DS,'/salvrType/getOne',{id:record.id},(response)=>{
			that.setState({
				checkedData:response.data,
				rodalShow : true,
				showHeader:true,
	...
	deleteClick(data){
		let numArr = this.refs.salvrtype.getSelectArr();
		let value=[];
		var that = this;
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/salvrType/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/salvrType/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/salvrType/getPage',object,
				(response)=>{	
					that.setState({	
...
export default NavConnect(Salvrtype);
