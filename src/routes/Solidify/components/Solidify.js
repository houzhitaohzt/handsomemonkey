...
import SolidifyPlug from './SolidifyPlug';
...
class Shelftype extends Component{
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100215/*箱型新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/contnrType/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow : true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
				...
				if(numArr.length > 0){
					...
					Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/contnrType/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		var that = this;
		let a = [];
		value.transTypeIds.map((e)=>{a.push(e.id)});
		delete value.transTypeIds;

		value=Object.assign({},data,value,{transTypeIds:a});
		
		apiPost(API_FOODING_DS,'/contnrType/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	onRowDoubleClick(record,index,checked){
		let that = this;
		apiGet(API_FOODING_DS,'/contnrType/getOne',{id:record.id},(response)=>{
			that.setState({
				datas:response.data,
				rodalShow : true,
				showHeader:true,
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/contnrType/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<SolidifyPlug DialogContent={this.state.DialogContent}
