...
export class  ContactDialog extends Component{
	...
	lbizPrnClick(){
		var that = this;
		if(this.props.data.record.beId){
			apiGet(API_FOODING_DS,'/entContact/getByBeIdDataTyId',{beId:this.props.data.record.beId,dataTyId:100},
				(response)=>{
					that.setState({
		...
		this.props.form.validateFields((error, value) => {
			if(error){
				console.log(error, value);
					...
					var contactList = [{name:value.email,linkTyId:80,dfutMark:true,id:defaultEmailId,optlock:defaultEmailOptlock,dataTyId:this.props.dataTyId},
				...
				delete value.skype;
				value = Object.assign({},that.props.data.record,value,{contactList:contactList});
	      		value = Object.assign({},value,{beId:this.props.id});
		    	apiPost(API_FOODING_DS,'/carrierLinkMan/save',value,(response)=>{
	...
	render(){
		...
		let checkedData = data.record||{};
		let common1 = <div></div>;
			if(checkedData.id){
				common1 =<AddMoreLanguage
									    menusetView={data.record}
	   ...
	   if(data.type == 2){
		...
		content = (
						<div className="contact-edit scroll" style={{overflow: 'inherit'}}>
													...
													<div className='form-group col-xs-6 col-md-6'>
														...
														<ConstMiniSelect form={this.props.form}
															isRequest={true}
				                                                 ...
				                                                 initValueOptions={[]}
				                                                 initialValue={
				                                                 	xt.initSelectValue(checkedData.lbizPrn, {
