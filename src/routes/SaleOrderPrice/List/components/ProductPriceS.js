...
class ProductPrice extends Component{
	...
	onSaveAndClose(){
		...
		//已选产品的id数组
		let ormbs = this.props.selectData.map((o) => ({mtlId:o.mtlId,offerId:o.billId}));
    ...
    btnClick(i,e,j){
        ...
        if(selectData.indexOf(i)== -1){
            selectData.push(i);
        }else{
            selectData.splice(e,1);
        }
        this.setState({
            selectData
        });
	...
	render(){
		...
		let {selectData,selectArr} =this.props;
		let  iconArray = [{type:'delete',onClick:this.deleteClick}];
		let  that = this;
		return (<FormWrapper showFooter={true} onSaveAndClose={this.onSaveAndClose} onCancel={this.onCancel} >
			<div className="productprice scroll" style={{marginBottom:'10px',maxHeight:'320px',overflowX:'hidden'}}>
			<div className="gtoupe" style={{margin:'0px',overflow:'hidden'}}>
				<div className='content' style={{paddingTop:'0px',margin:'0px'}}>
                        <div className='neirong' style={{borderRadius:'10px',cursor:'pointer',display:'inline-block',marginRight:'10px',position:'relative',borderTop:'0px'}}>
                             {
                                selectData.map((value,i)=>{
                                    return(<span key={i}
                                        className ={this.props.selectData.indexOf(value) != -1 ? 'height-l':''}
				...
				<div className="row" style={{marginBottom:'5px'}}>
					...
					<div className="form-group col-md-4 col-lg-4">
						<span style={{color:'#9facbd', marginRight: '20px'}}>{I18n.t(100411/*是否发邮件*/)}</span>
						 <Checkbox
                            defaultChecked
					...
					<div className="form-group col-md-4 col-lg-4">
                               <p className="client-price-content-show col-xs-4" style={{textAlign:'right'}}>{i18n.t(200457/*接受对象*/)}</p>
                               <ConstVirtualSelect
                                    pageSize={6}
                                    multi
                                    apiType={apiPost}
