...
class CommonForm extends Component{
	...
	onSaveAndClose = () => {
		...
		apiPost(API_FOODING_ERP,"/shipping/split",{billId:numArr[0].billId,nums:this.spliteArr},response => {
					...
					<div className={'row'}>
						<div className="form-group col-xs-6 col-md-6">
							...
							<div className={'col-md-9 col-lg-9'}>
								<p className={'paragraph shengyue'}>{numArr[0].mtlLcName}</p>
							</div>
						</div>
						<div className="form-group col-xs-6 col-md-6">
							<label className={'col-xs-3 col-md-3'}>{i18n.t(400035/*产品单位*/)}</label>
							<div className={'col-md-9 col-lg-9'}>
								<p className={'paragraph shengyue'}>{numArr[0].uomLcName}</p>
							</div>
						</div>
					</div>
...
const BoodNeedSplit = createForm()(CommonForm);

export default BoodNeedSplit;
