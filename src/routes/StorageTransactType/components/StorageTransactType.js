...
import StorageTransactTypePlug from './StorageTransactTypePlug';
import {I18n} from "../../../lib/i18n";
class StorageTransactType extends Component{
constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
			...
			showSaveAdd:true,
			showSaveClose:true,
			title:I18n.t(500012/*仓储事务类型新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/storOpert/getOne',{id:record.record.id},(response)=>{
			that.setState({
	...
	onRowDoubleClick(record,index,checked){
		...
		var that = this;
		apiGet(API_FOODING_DS,'/storOpert/getOne',{id:record.id},(response)=>{
			that.setState({
				checkedData:response.data,
				rodalShow : true,
				showHeader:true,
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/storOpert/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<StorageTransactTypePlug DialogContent={this.state.DialogContent}
...
export default NavConnect(StorageTransactType);
