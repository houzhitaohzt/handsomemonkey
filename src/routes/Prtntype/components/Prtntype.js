...
import PrtntypePlug from './PrtntypePlug';
...
class Prtntype extends Component{
	constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100174/*合伙功能新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/prtnType/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow:true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/prtnType/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/prtnType/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/prtnType/getPage',object,
				(response)=>{	
					that.setState({	
			...
			<div className={'client-body'} style={{height:this.state.scrollHeight}}>
				...
				<div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<PrtntypePlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Prtntype);
