...
export class AirPricePlug extends Component{
	...
	render(){
	    ...
	    // getFieldProps('billId', {
		   //    initialValue: this.props.otherData.billId || '',
	    // });
		let content = <div></div>;
		content =(
						<div className={'  girdlayout'} style={{height:"344px"}}>
							...
							<div className={'row'}>
								                <div className="form-group col-xs-6 col-md-6">
													...
													<input  
																...
																validateFirst: true,
																rules: [{required:true}],
																initialValue: String(getOne.qty || '')})}
							...
							<div className={'row'}>
								                <div className="form-group col-xs-6 col-md-6">
													...
													<input  
											   				 type="text"
																{...getFieldProps('purTaxPrc',{
																validateFirst: true,
																rules: [{required:true}],
																initialValue:String(getOne.purTaxPrc|| '')})}
																className={getFieldError("purTaxPrc") ?'col-xs-9 col-md-9 text-input-nowidth error-border':'col-xs-9 col-md-9 text-input-nowidth'}
													/>
												</div>
												<div className="form-group col-xs-6 col-md-6">
													...
													<Select
														animation='slide-up'
														placeholder=''
														className ={ getFieldError("purCnyId") ?'col-md-9 col-lg-9 currency-btn select-from-currency error-border':'col-md-9 col-lg-9 currency-btn select-from-currency'}
														...
														allowClear
														>
														{this.state.bizhongArray.map((o,i)=><Option key={i} objValue={{s_label:o.localName, purCnyId:o.id, purCnyLcName:o.localName, purCnyEnName:o.name}} title={o.name}>{o.localName}</Option>)}
													</Select>
												</div>
							</div>
							<div className={'row'}>
