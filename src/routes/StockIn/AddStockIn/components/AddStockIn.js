  		...
  		apiPost(API_FOODING_ERP,'/noticestock/save',Object.assign({},getOne,value),(response)=>{
	      			...
	      			if(isclose){
	      			   initAjax();
	      				...
	      				//  this.props.router.push({pathname:'/stockin/detail',query:{id:response.data}});
	      			}
	    },(error)=>{
            ...
            if (!error) {
                ...
                apiPost(API_FOODING_ERP, '/noticestock/save', param,
                    response => {
                        ...
                        let done = ()=>{
                        ServiceTips({text:response.message,type:'sucess'});
						that.props.navReplaceTab({name:I18n.t(500133/*入库通知单详情*/), component:I18n.t(500133/*入库通知单详情*/), url: '/stockin/detail'});
						this.props.router.push({pathname:'/stockin/detail',query:{id:response.data}});
