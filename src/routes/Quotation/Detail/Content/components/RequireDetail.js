...
export class ProductDetail extends Component{
    ...
    initProduct(billId){
    	apiGet(API_FOODING_ERP,'/saleoffer/mtl/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initMatou(){
    	apiGet(API_FOODING_ERP,'/saleoffer/marks/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initZhengshu(){
    	apiGet(API_FOODING_ERP,'/saleoffer/card/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initChuang(){
    	apiGet(API_FOODING_ERP,'/saleoffer/ship/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initYunshu(){
    	apiGet(API_FOODING_ERP,'/saleoffer/lsreqir/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initZhuangchuang(){
    	apiGet(API_FOODING_ERP,'/saleoffer/require/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initheYan(){
    	apiGet(API_FOODING_ERP,'/saleoffer/test/getList',{billId:this.props.id},(response)=>{
    		this.setState({
    ...
    initzhuangXiang(){
    	apiGet(API_FOODING_ERP,'/saleoffer/pakg/getList',{billId:this.props.id},(response)=>{
    		this.setState({
		...
		return (
			  	<div>
	               	<div style={{backgroundColor:'#f0f4f8',overflow:'hidden'}}>
												...
												return <div>
                                                    {
                                                        that.state.titleArray.map((value,i)=>{
                                                            if(data && data[value["id"]]){
                                                                  return <span style={{paddingRight:'10px'}} key={i}>{value["localName"]+' '+data[value["id"]]}</span>;  
                                                            }
                                                        })
                                                    }                                    
                                                </div>;
						...
						<div>
		                     	<OnlyreadyRuleTemplate Zindex={3} onCancel ={this.onCancel} title ={i18n.t(500079/*检验要求*/)} openDialog={this.handleClick}
				                     id={'37'}
				                     showHeader ={true}
					                   		columns ={
							                    [{
		            		...
		            		<div>
		                     	<OnlyreadyRuleTemplate  Zindex={3} onCancel ={this.onCancel} title ={i18n.t(200340/*装箱信息*/)} openDialog={this.handleClick}
				                     id={'37'}
				                     showHeader ={true}
					                   		columns ={
							                  [{
