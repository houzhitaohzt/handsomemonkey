...
class CommonForm extends Component{
    ...
    getEditOne = billDtlId => {
        ...
        apiGet(API_FOODING_ERP, '/saleoffer/ship/getOne', {id: billDtlId},
            response => {
	...
	onSaveAndClose(isAdd){
		this.props.form.validateFields((error, value) => {
			if(error){
				...
				apiPost(API_FOODING_ERP,'/saleoffer/ship/save',value,(response)=>{
		...
		let content = <div></div>
				content =(
						<div className={'  girdlayout'} style={{height:"344px"}}>
							...
							<div className={'row'}>
								                <div className="form-group col-xs-12 col-md-12">
													<label className={'col-xs-2 col-md-2'}><span>*</span>{i18n.t(500076/*船公司*/)}</label>
													<ConstMiniSelect form={this.props.form}
			                                                 pbj={{
			                                                     apiType: apiPost, host: API_FOODING_DS, uri:'/object/getMiniList',
			                                                     params: {
