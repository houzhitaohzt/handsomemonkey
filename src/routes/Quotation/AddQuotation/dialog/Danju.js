...
class CommonForm extends Component{
    ...
    getEditOne = billDtlId => {
        ...
        apiGet(API_FOODING_ERP, '/saleoffer/billreqir/getOne', {id: billDtlId},
            response => {
	...
	onSaveAndClose(isAdd){
		this.props.form.validateFields((error, value) => {
			if(error){
				...
				apiPost(API_FOODING_ERP,'/saleoffer/billreqir/save',value,(response)=>{
			...
			if(data.number == 0 || data.number == 1){
				content =(
						<div className={'  girdlayout'} style={{height:"344px"}}>
							...
							<div className={'row'}>
								                <div className="form-group col-xs-12 col-md-12">
													<label className={'col-xs-2 col-md-2'}><span>*</span>{i18n.t(100128/*单据要求*/)}</label>
													<ConstMiniSelect form={this.props.form}
			                                                 pbj={{
			                                                     apiType: apiPost, host: API_FOODING_DS, uri:'/object/getMiniList',
			                                                     params: {obj:'com.fooding.fc.ds.entity.BillRequ'}
