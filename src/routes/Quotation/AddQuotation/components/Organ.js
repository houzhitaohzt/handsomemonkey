				...
				<div className={'  girdlayout'}>
					<div className={'row'}>
							...
							<ConstMiniSelect form={this.props.form}
												isRequest={Boolean( getFieldValue('ccId',getOne).ccId)}
                                                 ...
                                                 pbj={{
                                                     ...
                                                     params: {partyId:getFieldValue('ccId', getOne).ccId,typeAttributeIds:["44"]}
                                                 }} fieldName="sorId"
                                                 initValueOptions={[]}
                                                 initialValue={
                                                 	xt.initSelectValue(getOne.sorId && getFieldValue('ccId', getOne).ccId === getOne.ccId, getOne, ['sorId', 'sorLcName', 'sorEnName'], "sor"+language, this.props.form)
                                                 }
                                                 optionValue={(da, di) => <Option key={di} objValue={{
                                                     sorId: da.id,
						...
						<div className="form-group col-md-3 col-lg-3">
							...
							<ConstMiniSelect form={this.props.form}
												isRequest={Boolean(getFieldValue('sorId', getOne).sorId)}
                                                 refreshMark={getFieldValue('sorId', getOne).sorId}
                                                 pbj={{
                                                     apiType: apiGet, host: API_FOODING_ES, uri: '/user/getListForPermissionsInParty',
                                                     params: {partyId:getFieldValue('sorId', getOne).sorId}
                                                 }} fieldName="saleStaffId"
                                                 initValueOptions={[]}
                                                 initialValue={
                                                 	xt.initSelectValue(getOne.saleStaffId && getFieldValue('sorId', getOne).sorId === getOne.sorId, getOne, ['saleStaffId', 'saleStaffLcName', 'saleStaffEnName'], "saleStaff"+language, this.props.form)
                                                 }
                                                 optionValue={(da, di) => <Option key={di} objValue={{
