...
import TermsOfTradePlug from './TermsOfTradePlug';
...
class TermsOfTrade extends Component{
	constructor(props){
		...
		this.columns = [{
			...
			title : I18n.t(100258/*交易条款类型*/),
			dataIndex : "incotmType",
			key : "incotmType",
			width : "13%",
			render(data,row,index){
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/incotm/getPage',object,
				(response)=>{	
					that.setState({	
				  ...
				  <div className="action-buttons">
					<div className={'key-page'}>
						...
						<Dialog width={926}  visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
							<TermsOfTradePlug DialogContent={this.state.DialogContent}
...
export default NavConnect(TermsOfTrade);
