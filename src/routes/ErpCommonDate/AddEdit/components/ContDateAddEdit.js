            ...
            if (error) {
               callback(I18n.t(500211/*主表信息不完善!*/))
                ...
                apiPost(API_FOODING_OA, '/activity/save', param, response => {
                    ...
                    this.props.router.push({pathname: '/cdateerp/edit', query: newQuery, state: {refresh: false}});
        ...
        form.validateFields((error, value) => {
            if (error) {
                ...
                apiPost(API_FOODING_OA, "/activity/save", valueone, response => {
                    ...
                    that.props.router.push({pathname: '/cdateerp/detail', query: newQuery, state: {refresh: true}});
                }, error => ServiceTips({text: error.message, type: 'error'}))
            }
        })
    }
