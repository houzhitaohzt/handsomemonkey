...
import CostNamePlug from './CostNamePlug';
import {I18n} from "../../../lib/i18n";
class CostName extends Component{
constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
			...
			showSaveAdd:true,
			showSaveClose:true,
			title:I18n.t(100275/*费用名称新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/costlvtr/getOne',{id:record.record.id},(response)=>{
			that.setState({
	...
	deleteClick(data){
				...
				if(numArr.length > 0){
					...
					Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/costlvtr/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/costlvtr/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	onRowDoubleClick(record,index,checked){
		var that = this;
		apiGet(API_FOODING_DS,'/costlvtr/getOne',{id:record.id},(response)=>{
			that.setState({
				checkedData:response.data,
				rodalShow : true,
				showHeader:true,
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/costlvtr/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<CostNamePlug DialogContent={this.state.DialogContent}
...
export default NavConnect(CostName);
