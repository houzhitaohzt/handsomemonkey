...
export class PageFrom extends Component{
	constructor(props){
		super(props);
		let that = this;
		...
		this.columns = [{
			...
			render(data,row,index){
				return (<div className="text-ellipsis">{row.rival['code']}</div>);
			}
		},{
			...
			width : "20%",
			render(data,row,index){
				return (<div className="text-ellipsis">{row.rival['localName']}</div>);
			}
		},{
			...
			width : "10%",
			render(data,row,index){
				return (<div className="text-ellipsis">{row.rival['country']}</div>);
			}
		},{
			...
			width : "20%",
			render(data,row,index){
				return (<div className="text-ellipsis">{row.rival['defaultWeb']}</div>);
			}
		},
		// {
		// 	title : "主联系人",
		...
		// 	width : "16%",
		// 	render(data,row ,index){
		// 		return (<div className="text-ellipsis">{row.rival.defaultEntContact ? row.rival.defaultEntContact.localName : ''}</div>);
		// 	}
		// },
		{
    ...
    deleteClick(e){
    	...
    	DeleteDialog(i18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
				...
				apiForm(API_FOODING_DS,'/tradruleCompetitor/delete',{id:ID},
					(response)=>{
	...
	getPage = ()=>{
		...
		apiGet(API_FOODING_DS,'/tradruleCompetitor/getPage',{sourceId:id,pageSize:pageSize,currentPage:currentPage},
			(response)=>{	
				let data = response.data.data.filter(o=>o['rival'])
				that.setState({	
					data: data,
...
export default NavConnect(PageFrom);
