...
export  class  ActivityDetail extends Component{
	...
	getOne(callBack){
		let that = this;
		apiGet(API_FOODING_ERP,'/charge/getOne',Object.assign({billId:that.state.billId,content: 499},),
			(response)=>{
		...
		return (
			<div className='activity-detail scroll' style={{height:this.state.scrollHeight}}>
				<AddNormal 
					...
					/>
					{ sourceType == 318 && this.props.location.query['id'] ?  
						<Baoxiao isShowChecked={true}
							id={this.state.id}
							getForm={this.getForm}
							onSaveAndClose={this.onSaveAndClose}
							getOneData={this.state.getOneData}
