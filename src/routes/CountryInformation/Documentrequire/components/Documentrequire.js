...
import DocumentrequirePlug from './DocumentrequirePlug'
...
export class Documentrequire extends Component{
	constructor(props){
		super(props);
        props.documentrequire && props.documentrequire(this);
		this.columns = [{
	...
	addClick(e){
		this.setState({
			...
			showHeader:true,
			DialogContent:1,
			title:I18n.t(100149/*单据要求新增*/)
		})
    }
   editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/tradruleBillrequ/getOne',{id:record.record.id},(response)=>{
			this.setState({
				rodalShow:true,
				showHeader:true,
				title:I18n.t(100150/*单据要求编辑*/),
				showSaveAdd:false,
				showSaveClose:true,
				DialogContent:3,
    ...
    deleteClick(data){
		...
		if(numArr.length > 0){
			...
			DeleteDialog(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/tradruleBillrequ/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/tradruleBillrequ/country/getPage',object,
				(response)=>{	
					that.setState({	
		...
		return (<div>
				...
				<div className={'client-body-single'}>
					...
					<Dialog visible={this.state.rodalShow} showHeader ={this.state.showHeader}  title={this.state.title} width={926}>
							<DocumentrequirePlug DialogContent={this.state.DialogContent}
