...
import SpecialrequirePlug from './SpecialrequirePlug'
//引入提示
...
export class Shipmark extends Component{
	constructor(props){
		super(props);
        props.specialrequire && props.specialrequire(this);
		this.columns = [{
	...
	addClick(){
		this.setState({
			...
			showHeader:true,
			DialogContent:1,
			title:I18n.t(100118/*特殊要求新增*/)
		})
    }
    editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/tradrulePropty/getOne',{id:record.record.id},(response)=>{
			this.setState({
				rodalShow:true,
				showHeader:true,
				title:I18n.t(100119/*特殊要求编辑*/),
				showSaveAdd:false,
				showSaveClose:true,
				DialogContent:3,
    ...
    deleteClick(data){
		...
		if(numArr.length > 0){
			...
			DeleteDialog(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/tradrulePropty/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/tradrulePropty/getPage',object,
				(response)=>{	
					that.setState({	
		...
		return (<div>
				...
				<div className={'client-body-single'}>
					...
					<Dialog visible={this.state.rodalShow} showHeader ={this.state.showHeader}  title={this.state.title} width={926}>
						<SpecialrequirePlug DialogContent={this.state.DialogContent}
