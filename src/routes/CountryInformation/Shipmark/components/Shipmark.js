...
import Shipmarkplug from './ShipmarkPlug'
...
export class Shipmark extends Component{
	constructor(props){
		super(props);
        props.shipmark && props.shipmark(this);
		this.columns = [{
	...
	addClick(e){
		this.setState({
			...
			showHeader:true,
			DialogContent:1,
			title:I18n.t(100109/*新增唛头*/)
		})
    }
   editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/tradruleShipmark/getOne',{id:record.record.id},(response)=>{
			this.setState({
				rodalShow:true,
				showHeader:true,
				title:I18n.t(100110/*唛头编辑*/),
				showSaveAdd:false,
				showSaveClose:true,
				DialogContent:3,
    ...
    deleteClick(data){
		...
		if(numArr.length > 0){
			...
			DeleteDialog(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/tradruleShipmark/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/tradruleShipmark/getPage',object,
				(response)=>{	
					that.setState({	
		...
		return (<div>
				...
				<div className={'client-body-single'}>
					...
					<Dialog visible={this.state.rodalShow} showHeader ={this.state.showHeader}  title={this.state.title} width={926}>
							<Shipmarkplug DialogContent={this.state.DialogContent}
