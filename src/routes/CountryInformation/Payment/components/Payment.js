...
import PaymentPlug from './PaymentPlug'
...
export class Payment extends Component{
	constructor(props){
		super(props);
        props.payment && props.payment(this);
		this.columns = [{
    ...
    editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/tradrulePayterm/getOne',{id:record.record.id},(response)=>{
			this.setState({
				rodalShow:true,
				showHeader:true,
    ...
    deleteClick(data){
		...
		if(numArr.length > 0){
			...
			DeleteDialog(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/tradrulePayterm/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/tradrulePayterm/getPage',object,
				(response)=>{	
					that.setState({	
		...
		return (<div>
				...
				<div className={'client-body-single'}>
					...
					<Dialog visible={this.state.rodalShow} showHeader ={this.state.showHeader}  title={this.state.title} width={926}>
							<PaymentPlug DialogContent={this.state.DialogContent}
