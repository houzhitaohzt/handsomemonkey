...
class ProductList extends Component{
	...
	deleteClick(){
		let numArr = this.refs.hotsaleproduct.getSelectArr();
		let tempString = "",id=[];
		if(numArr.length==0){
	...
	getPage(currentPage,order){
		...
		apiGet(API_FOODING_DS,'/platformHotMaterial/getPage',object,
				(response)=>{	
					that.setState({	
	...
	deletedFunc(ID){
		let that = this;
		apiForm(API_FOODING_DS,'/platformHotMaterial/delete',{id: ID},
			(response)=>{
