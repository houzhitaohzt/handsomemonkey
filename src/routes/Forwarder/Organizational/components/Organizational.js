...
export class ForwarderOrganizational extends Component {
    constructor(props) {
        super(props);
        ...
        if (data == i18n.t(100437/*删除*/)) {
            ...
            Confirm("删除节点后将无法恢复,您确定要删除吗?", {
                    ...
                    apiForm(API_FOODING_DS, '/enterpriseOrg/delete', {id: rightObj.orgCompanyId}, (response) => {
                        ...
                        $("#ForwardjOrgChart" + this.state.id).html("");
                        that.getChild();
                    }, (error) => {
    ...
    getChild = () => {
        ...
        apiGet(API_FOODING_DS, '/enterpriseOrg/getTreeById', {
            ...
            that.setState({treeData: response.data}, () => {
                ...
                showlist.attr('id', 'org1' + this.state.id);
                this.showall(response.data, showlist);
                $("#ForwardjOrgChart" + this.state.id).append(showlist);
                $("#org1" + this.state.id).jOrgChart({
                    chartElement: '#ForwardjOrgChart' + this.state.id,//指定在某个dom生成jorgchart
    ...
    getXY = (obj,type) => {
        if(type === 'onclick'){//点击
            ...
            this.props.router.push({pathname:'/forwarder/detail',query:{id:obj.data.orgCompanyId,index:'detail'},state: {refresh: true}});
            ...
            this.setState({
                ...
                document.getElementById('ForwarderOrganizational' + this.state.id).focus();
            })
        }
    };
...
export default NavConnect(ForwarderOrganizational);
