...
class CommonForm extends Component{
    ...
    getEditOne = billDtlId => {
        ...
        apiGet(API_FOODING_ERP, '/shipping/pakg/getOne', {id: billDtlId},
            response => {
	...
	onSaveAndClose(isAdd){
		this.props.form.validateFields((error, value) => {
			if(error){
				...
				apiPost(API_FOODING_ERP,'/shipping/pakg/save',value,(response)=>{
	...
	render(){
		...
		let content = <div></div>
				content =(
						<div className={'  girdlayout'} style={{maxHeight:"344px"}}>
						  	<div className={'row'}>
								                <div className="form-group col-xs-12 col-md-12">
													<label className={'col-xs-2 col-md-2'}><span>*</span>{i18n.t(100214/*箱型*/)}</label>
							...
							<div className={'row'}>
								<div className="form-group col-xs-12 col-md-12">
										<label className={'col-xs-2 col-md-2'}><span>*</span>{i18n.t(200341/*集装箱数*/)}</label>
													<input type='text' className={getFieldError("contQty")?'col-xs-10 col-md-10 text-input-nowidth error-border':'col-xs-10 col-md-10 text-input-nowidth'} 
														placeholder=""
