...
import CostlvtPlug from './CostlvtPlug';
...
class Costlvt extends Component{
	constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
			...
			showSaveAdd:true,
			showSaveClose:true,
			title:I18n.t(100272/*新增费用小类*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/costlvt/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow:true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
				...
				if(numArr.length > 0){
					...
					Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/costlvt/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/costlvt/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				that.refs.mainTable.clearChecked()
				ServiceTips({text:response.message,type:'success'});
				this.getPage();
		},(errors)=>{
			ServiceTips({text:errors.message,type:'error'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/costlvt/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<CostlvtPlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Costlvt);
