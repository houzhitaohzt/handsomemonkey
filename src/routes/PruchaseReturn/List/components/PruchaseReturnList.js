...
// ajax
import {apiGet,apiPost,apiForm,API_FOODING_ERP,API_FOODING_DS,language,pageSize,sizeList,toDecimal} from '../../../../services/apiCall';
import ServiceTips from '../../../../components/ServiceTips';

import { I18n } from '../../../../lib/i18n';

class PruchaseReturnList extends Component{
	constructor(props){
		super(props);
	...
	getPages = (currentPage,size,filter,order) => {
		...
		apiGet(API_FOODING_ERP,'/purorderreturn/getPage',params,(response)=>{
			this.setState({
	...
	deleteClick =() => {
		let numArr = this.refs.PruchaseReturn.getSelectArr(),billId;
		if(!numArr.length){
	...
	deletedFunc = (ID) => {	
		let that = this;
		apiForm(API_FOODING_ERP,'/purorderreturn/delete',{billId: ID},
			(response)=>{							
	...
	onRowDoubleClick(record,index,checked){
		...
		this.props.router.push({pathname:'/purorderreturn/detail',query:{id:record.billId},state: {refresh: true}});
	}
...
export default NavConnect(PruchaseReturnList);
