...
class PruchaseReturnFilterHeader extends Component{
	...
	show_hide_filter(){
		...
		this.setState({
			showFilter:classN
		},()=>this.props.expandFilter(null, this.refs.pruchaseInvoiceHarder.offsetHeight == 0?1:this.refs.pruchaseInvoiceHarder.offsetHeight))
	}
	search_more = () => {//收缩条件的展开和收缩
		...
		},()=>this.props.expandFilter(null, this.refs.pruchaseInvoiceHarder.offsetHeight))
	}
	...
	render(){
		...
		return(<div className={'clientcontact-list-header'} ref="pruchaseInvoiceHarder">{domFilter}</div>)
	}
}
