...
import './AttendanceResultsQuery.less';
export class AttendanceResultsqueryPlug extends Component{
	constructor(props){
		super(props);
		this.addSelect;
    ...
    renderConcatItem = (item, index)=> {
        ...
        return (
           <div className='AttendanceResultsquery-calender-a2' key={index}>
               {item.workStateId!==1?<span style={{float:'left',color:'#fff',backgroundColor:'#F00', width: '20px', height: '20px', color: '#fff', borderRadius: '4px',textAlign: 'center', lineHeight: '20px'}}>休</span>:<span style={{color:'#fff',backgroundColor:'blue', width: '20px', height: '20px', color: '#fff', borderRadius: '4px',textAlign: 'center', lineHeight: '20px'}}>班</span>}
                <div>{new Date(item.scheduleDate).Format('yyyy-MM-dd')}</div>
                <span>{item.remark}</span>
            </div>
        )
...
const ProductForm =createForm()(AttendanceResultsqueryPlug);
