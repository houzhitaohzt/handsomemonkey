...
export class MeasureCommon extends Component{
	...
	handleClick(e, data, target){
		...
		let element=React.createElement(DialogTempalte,
			{onSaveAndClose:this.props.onSaveAndClose,onCancel:this.props.onCancel,
				data:data,id:this.props.id,moduleIdArray:this.props.moduleIdArray,
				upload:this.props.upload,menusetView:this.props.menusetView
				
			});
		if(DialogTempalte){
			this.props.openDialog(e,data,element, this.refs["mainTable" + this.props.id].getSelectArr(),target);
		}else{
	...
	render(){
		...
		let arrayRightkey = [
	        {permissions:'menuset.add',type:i18n.t(100392/*新增*/),child:<div><i className='foddingicon fooding-add-icon3'></i>{i18n.t(100392/*新增*/)}</div>}
	    ];
	    let iconArray=[
	    	{permissions:'menuset.edit',action:i18n.t(100439/*编辑*/),classn:'foddingicon fooding-alter_icon2',title:i18n.t(100439/*编辑*/),data:{action:i18n.t(100439/*编辑*/),number:0,name:{title:this.props.title},id:this.props.id}},
	    	//{permissions:'menuset.add',action:i18n.t(100392/*新增*/),classn:'foddingicon fooding-add-icon3',title:i18n.t(100392/*新增*/),data:{action:i18n.t(100392/*新增*/),number:1,name:{title:this.props.title},id:this.props.id}},
	    	{permissions:'menuset.del',action:i18n.t(100437/*删除*/),classn:'foddingicon fooding-delete-icon3',title:i18n.t(100437/*删除*/),data:{action:i18n.t(100437/*删除*/),number:2,name:{title:this.props.title},id:this.props.id}}	
	    	];
		...
		if(this.props.data.length == 0){
			return (
			<RightKey 
				array={arrayRightkey.filter(o=>o['permissions'] ? permissionsBtn(o['permissions']) : 1 )} 
				handleClick={this.addClick.bind(this,{action:i18n.t(100392/*新增*/),name:{title:this.props.title},id:this.props.id,number:1})} isShowMenu={true} id={this.props.id} >
				<div style={{width: '100%',overflow:'hidden',borderRadius: '6px'}} className='menuset-measurement nodata'>
					<div>
						...
						{ permissionsBtn('menuset.add') ? 
							<span className='icon'><i className={'foddingicon fooding-add-icon3'} onClick={this.addClick.bind(this,{action:i18n.t(100392/*新增*/),name:{title:this.props.title},id:this.props.id,number:1})}></i></span>
							:
							''
						}
						</div>
			...
			<div style={{width: '100%',borderRadius: '6px',zIndex:this.props.Zindex,position:"relative"}} className='menuset-measurement'>
				...
				<Morekeys 
					iconArray= {iconArray.filter(o=>o['permissions'] ? permissionsBtn(o['permissions']) : 1 )} id={this.props.id} datat ={()=>{return Array.of(1)}} 
