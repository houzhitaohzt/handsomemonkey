...
export default class PageNow extends Component {
  ...
  onCancel(){
    ...
    // 恢复
    recoverHandle = ()=>NavBtnFunc({row:this.refs.product.getSelectArr(),message:{btn:'recover',isFolder:false},confirm:i18n.t(700094/*确定恢复?*/),callBack: this.getPage});
	...
	getPage = (search)=>{
        ...
        function Ajax(data={}){
            apiGet(API_FOODING_MAIL,'/box/getList',Object.assign({isDelete:true,userAddress:loginMessageNow['email'],followMark:followMark,color:color,pageSize:pageSize,currentPage:currentPage},that.state.searchData,data),
                (response)=>{
                    that.setState({
                        record: []
                    },function(){
    ...
    render(){
        ...
        return <div className="mail-list">
            ...
            <Table
                ...
                onRowDoubleClick={(row)=>detailFunc(row,'deleted')}
                contextMenuConfig={{
                    enable:true,
                    ...
                    {
                        onClick:(e,row)=>NavBtnFunc({row:Array.of(row['record']),message:{btn:'recover',isFolder:false},callBack: this.getPage}),
