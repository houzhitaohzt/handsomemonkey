...
import MyAttendancePlug from './MyAttendancePlug';
    ...
    complaintClick = attendId => {
        ...
        apiGet(API_FOODING_HR,"/explainRegister/getAttendaceById", {attendanId: attendId}, response => {
            this.setState({
                ...
                dialogContent: React.createElement(require('../../../Home/Dialog/PunchEditDialog').default, {
							...
							this.state.data.map((dataItem,i)=>{
								...
								<div className="train" key={i}>
									<div className="top">
                                        ...
                                        {dataItem["valid"]?<span style={{flex:2}}>
										    <span onClick={this.complaintClick.bind(this,dataItem['id'])} style={{marginRight:'20px',textDecoration:'underline',color:' #0066cc',cursor:'pointer'}}>申诉</span>
									...
									<div className="bottom">
										<span className="font-hide" style={{flex:2,marginLeft:'50px'}}>
											...
											<span style={{color:'#747d89'}} title={dataItem["overTimeLeave"] + dataItem["overTimePay"]} >{dataItem["overTimeLeave"] + dataItem["overTimePay"]}</span>
										</span>
										<span className="font-hide" style={{flex:2}}>
