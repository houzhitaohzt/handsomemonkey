...
class Charge extends Component{
	...
	addClick(){
		...
		navAddTab({id:13,name:i18n.t(200409/*费用单添加*/),component:i18n.t(200409/*费用单添加*/),url:'/charge/add'});
		this.props.router.push('/charge/add');
	}
	...
	onRowDoubleClick(record,index,checked){
		...
		this.props.router.push({ pathname: '/charge/detail',query:{id:record.billId,billType:record.billType}});
	}
	handleResize(height){
	...
	getPage(sData=null){
		...
		// 保存 请求
		function ajax(){
			apiGet(API_FOODING_ERP,'/charge/getPage',Object.assign({pageSize: that.state.pageSize,currentPage: that.state.currentPage, billType: 500},that.state.sData),
				(response)=>{				
					that.setState({	
