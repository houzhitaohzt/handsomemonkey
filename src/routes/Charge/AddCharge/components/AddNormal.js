...
class Addnormal extends Component{
						...
						that.setState({ billId: response.data },function(){
							...
							if( type == 'object' ) {
								Confirm(i18n.t(500100/*保存成功, 是否跳转到详情界面?*/), { timing: 5,done:()=>{
									navReplaceTab({id:13,name:I18n.t(200410/*费用单详情*/),component:I18n.t(200410/*费用单详情*/),url:'/charge/detail'}); 
									router.push({ pathname: '/charge/detail',query:{id:location.query['id'] || that.state['billId'],billType:getOneData['billType']}});								
								}});
							} else{
	...
	onBack() {
		...
		let {getOneData} = this.props;
		if(billId){
			this.props.navReplaceTab({name:I18n.t(200410/*费用单详情*/),component:I18n.t(200410/*费用单详情*/),url:'/charge/detail'});
			this.props.router.push({pathname: '/charge/detail', query: {id: billId,billType:getOneData['billType']}});
		} else {
			this.props.navReplaceTab({name:I18n.t(200406/*费用单*/),component:I18n.t(200406/*费用单*/),url:'/charge/list'});
			this.props.router.push({pathname: '/charge/list'});
		}
	}
		...
		return(
			<div className={'addnormal'}>
				...
				<div className={'  girdlayout'}>
					...
					<div className={'row'}>
						<div className="form-group col-md-3 col-lg-3">
							<label className={'col-md-4 col-lg-4'}>{i18n.t(200403/*费用合计*/)}</label>
							<input type="text"
								readOnly  
								className ={'col-md-8 col-lg-8 text-input-nowidth'}		
								placeholder={(getOneData.costAmt) || i18n.t(100378/*自动生成*/)} 
