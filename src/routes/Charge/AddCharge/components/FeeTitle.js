...
export class ProductDetail extends Component{
		...
		if(data.number == 2){
			...
			if(data.selectArr.length>1){
				...
				Confirm(i18n.t(500093/*删除后无法撤回,你确定要删除此订单吗？*/), {
						...
						apiForm(API_FOODING_ERP,'/charge/dtl/delete',{id: data.selectArr[0].billDtlId},
							(response)=>{	
			...
			// 编辑
			apiGet(API_FOODING_ERP,'/charge/dtl/getOne',{id:data.record.billDtlId},
				(response)=>{
					let dialogTitle= data.action+data.name.title;
					this.setState({
						visible:true,
	...
	getPage(){
		...
		let that = this;
		apiGet(API_FOODING_ERP,'/charge/dtl/getList',{billId: that.props.getOneData.billId || that.props.id || ''},
			(response)=>{	
				that.setState({	
					data: response.data,
				});
