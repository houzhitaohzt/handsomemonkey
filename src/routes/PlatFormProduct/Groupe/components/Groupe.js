...
export class Groupe extends Component{
    ...
    deleteNode = (ids) => {
        if(!ids.length) {
            ...
            Confirm(i18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
                    ...
                    apiForm(API_FOODING_DS,'/platformMaterial/deleteDataMulDiv2s',{id:this.state.id,dataMulDiv2ids:ids}, response => {
    ...
    /*getInitData = () => {
        let that = this;
        apiGet(API_FOODING_DS,'/platformMaterial/getOne',{id:this.state.id},response => {
            let dataMulDiv1s = response.data.dataMulDiv2s || [];
            let selectArr = dataMulDiv1s.map((o,i) => {return {selected:false,id:o.id,i:i,isClick:false}});
            that.setState({
                data:dataMulDiv1s,
    ...
    render(){
        let  iconArray = [{type:'add',onClick:this.addClick,permissions:'platform.mtl.dtl.add'},
        {type:'delete',onClick:this.deleteAllClick,permissions:'platform.mtl.dtl.del'}];
...
/*
    ...
    <i className='foddingicon fooding-menu_delete_32' onClick={this.cleanClick.bind(this,value,i)}></i>
*/
