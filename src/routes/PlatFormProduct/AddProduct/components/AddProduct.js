...
export  class  AddProduct extends Component{
	  		...
	  		form.validateFields((error, value) => {
	  			if(error){
					...
					let valueone = Object.assign({},params,{pPStdId:a});
	  				apiPost(API_FOODING_DS,'/platformMaterial/save',valueone,response => {
	  					this.productData = response.data;
                        this.props.router.push({pathname: '/platform/product/edit', query: {id: this.productData}, state: {refresh: false}});
        ...
        if(data.number ==2){
            ...
            Confirm(tempString, {
					...
					if(data.id == 26){//产品别名删除时,不要id,因此id没有作用
						...
						apiForm(API_FOODING_DS,'/platformMaterial/deleteExtNames',{id:this.productData,extNames:extName},response => {
							ServiceTips({text:response.message,type:'success'})
							this.getEditOne(this.productData);
						})
        			}
        			/*if(data.id == "26"){
	...
	// //进入新增页面时进行初始化
	// getData(){
	// 	apiGet(API_FOODING_DS,'/platformMaterial/getInit',{},(response)=>{
	// 		let	{mtlTypes,chargePeople,contnrTypes,dataDivs,loginStaff,pProcess,rptMtls,simProStands} = response.data;
	// 		this.setState({
	// 			mtlTypes:mtlTypes,
	// 			chargePeople:chargePeople,
	// 			contnrTypes:contnrTypes,
	// 			dataDivs:dataDivs,
	// 			loginStaff:loginStaff,
	// 			pProcess:pProcess,
	// 			rptMtls:rptMtls,
	// 			simProStands:simProStands
	// 		})
	// 	},(error)=>{
	// 		ServiceTips({text:error.message,type:'error'});
	// 	})
	// }
  		...
  		form.validateFields((error, value) => {
  			if(error){
				...
				/*if(contnrLoadings.length == 0){
  				...
  				apiGet(API_FOODING_DS,"/platformMaterial/getOne",{id:this.productData},response => {
  					let params = this.props.form.getFieldsValue();
	  				let a = [];
					params.pPStdId.map((e)=>{a.push(e.id)});
					delete params.pPStdId;
	  				let valueone = Object.assign({},that.props.form.getFieldsValue(),{id:this.productData,optlock:response.data.optlock,pPStdId:a});
	  				apiPost(API_FOODING_DS,'/platformMaterial/save',valueone,response => {
	  					ServiceTips({text:response.message,type:'success'});
	  					let {navAddTab,navReplaceTab} = that.props;

						navReplaceTab({name:I18n.t(100427/*平台产品*/) + I18n.t(100097/*详情*/),component:I18n.t(100427/*平台产品*/) + I18n.t(100097/*详情*/),url:'/platform/product/detail'});
						that.props.router.push({pathname:'/platform/product/detail',query:{id:this.productData}});
  	...
  	backClick(){
		...
		navReplaceTab({name:I18n.t(100427/*平台产品*/),component:I18n.t(100427/*平台产品*/),url:'/platform/product/list'});
		this.props.router.push('/platform/product/list');
  	}	

	//产品规格细分 新增和编辑保存
	onMtlQailtemSaveAndClose(value,data){
		value = Object.assign({},value,{mtlId:this.productData,dataTyId:25})
	...
	//产品细分初始化数据拉取
	getMtlQailtemPages(){
		apiGet(API_FOODING_DS,'/mtlQaitem/getPage',{mtlId:this.productData,pageSize:100,currentPage:1,column:'id',order:'desc',dataTyId:25},response => {
                ...
                this.setState({mtlQaitems, businessOne:Object.assign({}, this.state.businessOne, {specTxt:specTxt})});
			},error => {
	...
	onMeasumSaveAndClose(value,data){
		value = Object.assign({},value,{sourceId:this.productData,dataTyId:25})
		//data为空对象，表示是新增,否则表示是编辑		
		if(JSON.stringify(data) !== "{}"){
	...
	//计量单位初始化拉去数据
	getMeasumPage(){
		apiGet(API_FOODING_DS,'/measum/getPage',{sourceId:this.productData,pageSize:100,currentPage:1,column:'id',order:'desc',dataTyId:25},response => {
				this.setState({
					measums:response.data.data || []
				})
			},error => {
	...
	getContnrLoadingspage(){
		apiGet(API_FOODING_DS,'/contnrLoading/getPage',{sourceId:this.productData,pageSize:100,currentPage:1,column:'id',order:'desc',dataTyId:25},response => {
				this.setState({
					contnrLoadings:response.data.data || []
				})
			},error => {
	...
	onPackSaveAndClose(value,data){
		...
		//data为空对象，表示是新增,否则表示是编辑		
		if(JSON.stringify(data) !== "{}"){
			value = Object.assign({},value,{id:data.id,optlock:data.optlock,localName:data.packaging.localName,dataTyId:25})
		}
		//apiPost 保存数据
	...
	//产品包装 初始化
	getPackPage(){
		apiGet(API_FOODING_DS,'/pack/getPage',{sourceId:this.productData,pageSize:100,currentPage:1,column:'id',order:'desc',dataTyId:25},response => {
				this.setState({
					packs:response.data.data || []
				})
			},error => {
	...
	onMaterialExtNamesSaveAndClose(value,data){
		let that = this;
		value = Object.assign({},value,{id:this.productData,dataTyId:25})
		//apiPost 保存数据
		apiForm(API_FOODING_DS,'/platformMaterial/saveExtNames',value,response => {	
			ServiceTips({text:response.message,type:'success'})
			that.getEditOne(this.productData);
		},error => {
			ServiceTips({text:error.message,type:'error'})
		})
		this.setState({visible:false});
	...
	/*getMaterialExtNamesPage(){
		apiGet(API_FOODING_DS,'/materialExtName/getPage',{mtlId:this.productData,pageSize:100,currentPage:1,column:'id',order:'desc',dataTyId:25},response => {
				this.setState({
    ...
    getEditOne = id => {
	    if( !id) return;
	    this.productData = id;
        apiGet(API_FOODING_DS, '/platformMaterial/getOne', {
            id: id
        }, response => {
	...
	*/
	...
	render(){
		...
		const { mtlTypes,chargePeople,contnrTypes,dataDivs,loginStaff,pProcess,rptMtls,simProStands,
			psSubdivision,
			pPackaging,
			pAlias,
			measuringUnit,
			boxTypeNumber,
			BOMmaintenance,
			businessOne} = this.state;
		let {form} = this.props;
		let materialExtname = (businessOne && businessOne.extNames || []).map( (e,idx) => ({id:idx,extName:e})) || [];
		return (
		            	...
		            	<div className = 'col' style={{paddingLeft:0}}>
		                    ...
		                    <Measurement 
			                    ...
			                    Zindex={5}
	                             ...
	                             params={{dataTyId:25,sourceId:this.productData}}
