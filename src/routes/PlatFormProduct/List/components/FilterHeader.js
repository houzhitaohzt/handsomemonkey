...
class ProductFilterHeader extends Component{
	...
	show_hide_filter(){
		...
		this.setState({
			showFilter:classN
		},()=>this.props.expandFilter(null, this.refs.platFormProharder.offsetHeight == 0?1:this.refs.platFormProharder.offsetHeight))
	}
	search_more(){//收缩条件的展开和收缩
		...
		},()=>this.props.expandFilter(null, this.refs.platFormProharder.offsetHeight))
	}
	// 查找
	search(){
		this.props.getPage();
	...
	render(){
		...
		return(<div className={'product-list-header'} ref="platFormProharder">{domFilter}</div>)
	}
}
