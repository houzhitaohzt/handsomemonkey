...
import TransroutPlug from './TransroutPlug';
...
class Transrout extends Component{
	constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
			...
			showSaveAdd:true,
			showSaveClose:true,
			title:I18n.t(100152/*新增航线*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/transRout/getOne',{id:record.record.id},(response)=>{
			this.setState({
				rodalShow:true,
				showHeader:true,
				title:I18n.t(100153/*航线编辑*/),
				showSaveAdd:false,
				showSaveClose:true,
				DialogContent:3,
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/transRout/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		var that = this;
		let a = [];
		value.statns.map((e)=>{a.push(e.id)});
		delete value.statns;

		value=Object.assign({},data,value,{statns:a});
		apiPost(API_FOODING_DS,'/transRout/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	onRowDoubleClick(record,index,checked){
		this.setState({
			...
			showSaveClose:false,
			DialogContent:5,
			title:I18n.t(100154/*航线详情*/),
			checkedData:{record:record}
		})
	}
	getInfo(){
		var that = this;
		apiGet(API_FOODING_DS,'/transRout/getInit',{},(response)=>{
			that.setState({
				info:response.data
			});
		},(errors)=>{
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/transRout/getPage',object,
				(response)=>{	
					that.setState({	
			...
			<div className={'client-body'} style={{height:this.state.scrollHeight}}>
				...
				<div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<TransroutPlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Transrout);
