...
import UnitofmeaPlug from './UnitofmeaPlug';
...
class Unitofmea extends Component{
	constructor(props){
		super(props);
		...
		this.columns = [{
			...
			title :  I18n.t(100004/*单位类型*/),
			dataIndex : "unitofmeaType",
			key : "unitofmeaType",
			width : "20%",
			render(data,row,index){
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100041/*计量单位新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/unitofmea/getOne',{id:record.record.id},(response)=>{
			that.setState({
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/unitofmea/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/unitofmea/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/unitofmea/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<UnitofmeaPlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Unitofmea);
