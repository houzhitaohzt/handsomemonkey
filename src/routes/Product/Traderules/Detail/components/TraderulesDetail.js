...
export class TraderulesDetail extends Component{
        ...
        if(data.number== 2){
            ...
            if(!(data && data.record || data.selectArr.length == 1)){
             ...
             Confirm(tempString, {
                    ...
                    if(data.id == "client-detail-zhengshu"){//证书
                        //id = "client-detail-zhengshu"  表示是证书 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-jianzhuang"){
                    	 //id = "client-detail-jianzhuang"  表示是监装机构 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-maitou"){
                    	 //id = "client-detail-maitou"  表示是唛头 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-teshu"){
                    	 //id = "client-detail-teshu"  表示是特殊要求 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-fanqingxiao"){
                    	 //id = "client-detail-fanqingxiao"  表示是反倾销 此处id和传过去的id一样
                         apiForm(API_FOODING_DS,'/tradruleAntidump/delete',{id:id},response => {
                            ServiceTips({text:response.message,type:'success'});
                            that.fanqingxiaoInitData();
                        },(error) => {
                            ServiceTips({text:error.message,type:'error'});
                        })
                    }else if(data.id == "client-detail-tuopan"){
                    	 //id = "client-detail-tuopan"  表示是托盘要求 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-shuixiang"){
                    	 //id = "client-detail-shuixiang"  表示是税项 此处id和传过去的id一样
                    ...
                    }else if(data.id == "client-detail-jinzhi"){
                    	 //id = "client-detail-jinzhi"  表示是禁止销售 此处id和传过去的id一样
                         apiForm(API_FOODING_DS,'/tradruleAntidump/delete',{id:id},response => {
                            ServiceTips({text:response.message,type:'success'});
                            that.jinzhiInitData();
                        },(error) => {
   ...
   zhengshuSaveAndClose(value,data){
    ...
    jianzhuangSaveAndClose(value,data,isAdd){
    	let that = this;
        //value = Object.assign({},{id:this.state.id,dataTyId:20},value);
    ...
    jianzhuangInitData(){
    	let that = this;
        apiGet(API_FOODING_DS,'/tradruleInspct/getPage', {sourceId:this.state.platformMtlId,cntryId:this.state.cntryId,dataType:25},
         response => {
         	  that.setState({
	...
	render(){
		...
		return(<div className='scroll' style={{overflow:'auto',height:'600px'}}>
                <div className='col'>
