...
class ProviderList extends Component{
	...
	initHeadTable(e){
		...
		// },
		{
			...
			key : "defaultContact",
			width : "15%",
			// template:<input   name={"defaultContact"} style={{width:'100%'}} 	type='text' className='text-input'/>,
			render(data,row,index){
				if( !data ) return '';
				return <Card type="providerLinkMan" data={row['defaultContact']} router={that.props.router}/>;
			}
		},{
	...
	partyClick(e){
		...
		let element=React.createElement(content,{ident:'provider',onSaveAndClose:this.onSaveAndClose,onCancel:this.onCancel,getPage:this.getPage});
		this.setState({
    		showDilaog : true,
	...
	onRowDoubleClick(record,index,checked){
		...
		this.props.router.push({pathname:'/provider/detail',query:{id:record.id,index:'detail'},state: {refresh: true}});
	}
	...
	render(){
		...
		return(<div>
			...
			<div ref={"provider"} className={'client-body'} style={{height:this.state.scrollHeight}}>
