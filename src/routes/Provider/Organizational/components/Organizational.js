...
export class ProviderOrganizational extends Component {
    constructor(props) {
        super(props);
        ...
        if (data == i18n.t(100437/*删除*/)) {
            ...
            Confirm("删除节点后将无法恢复,您确定要删除吗?", {
                    ...
                    apiForm(API_FOODING_DS, '/enterpriseOrg/delete', {id: rightObj.orgCompanyId}, (response) => {
                        ...
                        $("#ProviderjOrgChart" + this.state.id).html("");
                        that.getChild();
                    }, (error) => {
    ...
    getChild = () => {
        ...
        apiGet(API_FOODING_DS, '/enterpriseOrg/getTreeById', {
            ...
            that.setState({treeData: response.data}, () => {
                ...
                showlist.attr('id', 'org3' + this.state.id);
                this.showall(response.data, showlist);
                $("#ProviderjOrgChart" + this.state.id).append(showlist);
                $("#org3" + this.state.id).jOrgChart({
                    chartElement: '#ProviderjOrgChart' + this.state.id,//指定在某个dom生成jorgchart
    ...
    getXY = (obj,type) => {
        if(type === 'onclick'){//点击
            ...
            navAddTab({id:'provider-detail',name:i18n.t(200935/*供应商详情*/),component:i18n.t(200935/*供应商详情*/),url:'/provider/detail'});
            this.props.router.push({pathname:'/provider/detail',query:{id:obj.data.orgCompanyId,index:'detail'},state: {refresh: true}});
            ...
            this.setState({
                ...
                document.getElementById('ProviderOrganizational' + this.state.id).focus();
            })
        }
    };
...
export default NavConnect(ProviderOrganizational);
