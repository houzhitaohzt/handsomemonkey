...
import FundtypePlug from './FundtypePlug';
...
class Fundtype extends Component{
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100182/*款项类型新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/fundType/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow:true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
				...
				if(numArr.length > 0){
					...
					Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/fundType/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/fundType/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/fundType/getPage',object,
				(response)=>{
					that.setState({
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<FundtypePlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Fundtype);
