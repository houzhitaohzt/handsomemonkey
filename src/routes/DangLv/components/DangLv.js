...
import DangLvPlug from './DangLvPlug';
...
class DangLv extends Component{
	constructor(props){
		super(props);
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title:I18n.t(100080/*危险等级新增*/)
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/dangLv/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow : true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/dangLv/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/dangLv/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className={'client-body-single'}>
				   <div className="action-buttons">
					...
					<Dialog width={926} visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
						<DangLvPlug DialogContent={this.state.DialogContent}
...
export default NavConnect(DangLv);
