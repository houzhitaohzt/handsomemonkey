...
class ForwarderContactList extends Component{
	...
	let content=require('./ForwarderContactListEdit').default;
					let element=React.createElement(content,{
						...
						onCancel:this.onCancel,info:{},checkedData:{},
						data:null,
						upload:this.getPage
					});
			    	this.setState({
				...
				let element=React.createElement(content,
					{
						...
						upload:this.getPage,
						info:this.state.info,data:{record:response.data,type:2}
				})
					that.setState({
	...
	getPage(currentPage,pageS){
		...
		let value = Object.assign(page,this.state.searchVal,{dataTyId:140},that.normalRef.getForm());
	...
	onRowDoubleClick(record,index,checked){
		...
		navAddTab({id:'forwardercontact-detail',name:I18n.t(300022/*服务机构联系人详情*/),component:I18n.t(300022/*服务机构联系人详情*/),url:'/servcon/detail'});
		this.props.router.push({pathname:'/servcon/detail',query:{id:record.id,sourceId:record.enterprise.id}});
	}
	...
	render(){
		...
		let {record,currentPage} = this.state;
			...
			meun = [{
					permissions:'servbecontact.edit',
				...
				<div className={'client-body-single'}>
				  <div className="action-buttons">
					...
					<Table
						...
						scroll={{x:true,y:this.state.scroll-10}}
