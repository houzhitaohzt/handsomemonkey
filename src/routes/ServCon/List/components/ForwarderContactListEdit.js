...
export class  ContactDialog extends Component{
	...
	lbizPrnClick(){
		...
		if(this.props.form.getFieldValue("beId")){
			...
			apiGet(API_FOODING_DS,'/entContact/getByBeIdDataTyId',{beId:beId,dataTyId:140},
				(response)=>{
					that.setState({
	...
	clientClick(){
		var that = this;
		apiPost(API_FOODING_DS,'/object/getMiniList',{obj:"com.fooding.fc.ds.entity.ServBe"},
			(response)=>{
			...
			if(error){
				console.log(error, value);
	      		...
	      		if(this.props.data){
					...
					var contactList = [{name:value.email,linkTyId:80,dfutMark:true,id:defaultEmailId,optlock:defaultEmailOptlock,dataTyId:140},
					{name:value.defaultMobile,linkTyId:100,dfutMark:true,id:defaultMobileId,optlock:defaultMobileOptlock,dataTyId:140},
					{name:value.skype,linkTyId:110,dfutMark:true,id:defaultSkypeId,optlock:defaultSkypeOptlock,dataTyId:140}];
					delete value.email;
					...
					that.props.onSaveAndClose(value,that.props.data.record);
				}else {
					var contactList = [{name:value.email,linkTyId:80,dfutMark:true,dataTyId:140},
					{name:value.defaultMobile,linkTyId:100,dfutMark:true,dataTyId:140},
					{name:value.skype,linkTyId:110,dfutMark:true,dataTyId:140}];
					delete value.email;
					delete value.defaultMobile;
					delete value.skype;
					value=Object.assign({},value,{contactList:contactList});
				}
				value = Object.assign({},value,{dataTyId:140});
			...
			content = (
						<div className="contact-edit scroll">
										<div className="girdlayout">
											...
											<div className="row">
												<div className='form-group col-xs-6 col-md-6'>
													<label className={'col-xs-3 col-md-3'}><span>*</span>{i18n.t(100313/*服务机构*/)}</label>
													<ConstVirtualSelect
													    form={this.props.form}
													    apiType={apiPost}
													    fieldName='beId'
													    apiUri='/enterprise/search?dataTyId=90&attrs=country._id,country.name,country.nameValues'
													    async={true}
													    apiParams='keyword'
														...
														<ConstMiniSelect form={this.props.form}
															isRequest={Boolean(getFieldValue('beId',{}).beId || (checkedData.enterprise&& checkedData.enterprise.id))}
			                                                 ...
			                                                 pbj={{
			                                                     ...
			                                                     params: {beId:getFieldValue('beId',{}).beId || (checkedData.enterprise && checkedData.enterprise.id),dataTyId:140,id:checkedData.id}
			                                                 }} fieldName="lbizPrnId"
			                                                 initValueOptions={[]}
			                                                 initialValue={
