...
export class Clentlist extends Component {
    ...
    partyClick() {
  		...
  		let element=React.createElement(content,{ident:'client',onSaveAndClose:this.onSaveAndClose,onCancel:this.onCancel,getPage:this.getPages});
  		this.setState({
    		showDilaog : true,
    ...
    getPages(currentPage, size, filter, order) {
        ...
        filter.assignTime = filter.assignTime ? filter.assignTime + ' 00:00:00' : filter.assignTime;

        let params = Object.assign({}, this.searchField, { currentPage: currentPage, pageSize: size,customerFiltrateId:this.state.clientFiler}, filter, order, this.filterData);
    ...
    // 领取公共客户 
    clientClick = ()=> {
        let IDs = this.refs.mainTable.getSelectArr().map(o => o.id);
     ...
     * */
    calendarClick = () => {
        let selectedArr = this.refs.mainTable.getSelectArr();
        if(!selectedArr.length) return;
        let currentTime = new Date(),start,end;
        start = this.getStartTime();
        end = new Date(new Date(start).getTime() + 1000*60*60*1).Format('yyyy-MM-dd hh:mm:ss');
        let activityDtos = selectedArr.map( e => ({
            router:"/client/detail",
            salBeId:e.id,
            salBeLcName:e.localName,
            stateId:"10",
            stateName:i18n.t(200616/*未处理*/)
        })); //i18n.t(200619/*已处理*/)
        let content = require('./CalendarDailog').default;
        let element = React.createElement(content, {onSaveAndClose: this.onSaveAndClose, onCancel: this.onCancel, start:start, end:end, activityDtos:activityDtos});
        this.setState({
            showDilaog: true,
                    ...
                    <div className="client-body-single">
                        <div className="action-buttons">
                            <div className="key-page">
                                ...
                                />
                                <FilterHeader getPage ={this.getPages} expandFilter={this.handleResize} searchCustomer={this.searchCustomer} formCall={form=>this.searchForm=form} onfilterChange={this.onfilterChange}/>
