...
import {I18n} from "../../../../../lib/i18n";
//引入ajax请求
import {API_FOODING_DS, apiForm, apiGet, apiPost, getUser} from "../../../../../services/apiCall";
...
export class CDetailsContent extends Component{
	constructor(props) {
       ...
       this.onSaveAndClose=this.onSaveAndClose.bind(this);
        ...
        //禁止停靠港口 保存并新增
        this.ondisTradruleStatnSaveAndClose = this.ondisTradruleStatnSaveAndClose.bind(this);
        //港口要求 保存并关闭
        this.ondisTradruleStatnSaveAdd=this.onTradruleStatnSaveAdd.bind(this);
        ...
        this.ondisTradruleStatnInitData=this.ondisTradruleStatnInitData.bind(this);//禁止停靠港口
    ...
    handleClick = (e, data, Template) => {
        ...
        if(data.number== 2){
            ...
            Confirm(tempString, {
                    ...
                    }else if(data.id == "client-detail-distradruleStatn"){//禁止停靠港口
                        apiForm(API_FOODING_DS,'/tradruleForbidStatn/delete',{id:id},(response)=>{
                            ServiceTips({text:response.message,type:'success'});
                            that.ondisTradruleStatnInitData();
                        },(error) => {
                            ServiceTips({text:error.message,type:'error'});
                        })
                    }
    ...
    //禁止停靠港口 保存并关闭
    ondisTradruleStatnSaveAndClose(value,data){
	    debugger
        let that = this;
        value = Object.assign({},{beId:this.state.id,dataTyId:30},value);
        if(JSON.stringify(data) !== "{}"){//表明是编辑
        ...
        apiPost(API_FOODING_DS,'/tradruleForbidStatn/save',value,(response) => {
            ServiceTips({text:response.message,type:'success'});
            that.ondisTradruleStatnInitData();
        },(error)=>{
            ServiceTips({text:error.message,type:'error'});
    ...
    //禁止停靠港口 保存并新增
    ondisTradruleStatnSaveAdd(value,data){
        let that = this;
        value = Object.assign({},{dataTyId:30},value);
        apiPost(API_FOODING_DS,'/tradruleForbidStatn/save',value,(response) => {
            ServiceTips({text:response.message,type:'success'});
            that.onTradruleStatnInitData();
        },(error)=>{
    ...
    onTradruleStatnInitData(){
        let that = this;
        apiGet(API_FOODING_DS,'/tradruleStatn/getList',{sourceId:this.state.id}, response => {
            that.setState({
    ...
    //禁止停靠港口
    ondisTradruleStatnInitData(){
        let that = this;
        apiGet(API_FOODING_DS,'/tradruleForbidStatn/getList',{sourceId:this.state.id}, response => {
            that.setState({
                distradruleStatnList:response.data
            })
        }, error => console.log(error.message))
    }
    //运输要求
    ...
    getPage(){
        ...
        this.ondisTradruleStatnInitData();
    }
    handleResize(height){
        let padding =  225;
	...
	render(){
        ...
        const {addressList,bankacctList,distradruleStatnList,contactList,cstmSalList,tradruleBillrequList,tradrulePaytermList,tradruleStatnList,tradruleTransbeList,TransportRequireList} = this.state || this.props;
                   ...
                   <MeasureCommon 
                    ...
                    <Template1 
                        ...
                        title={I18n.t(100140/*组织*/)} onSaveAndClose={this.onSaveAndClose}  tempArray={[{
                            key:I18n.t(100243/*集团*/),value:customer.cluster},
                            {key:I18n.t(100486/*公司*/),value:customer.company},
                            {key:I18n.t(100238/*部门*/),value:customer.department},
                            {key:I18n.t(100361/*分管人*/),value:customer.staff?(<b>{String(customer.staff)}<i className={'foddingicon fooding-approve'} title={I18n.t(100428/*变更记录*/)} onClick={this.onUpdateClick.bind(this,this.state.id)} style={{marginLeft:'5px'}}></i></b>):""}]}
                    />
                    <Template1 
                        menuList={[
                            {permissions:'clien.edit',type:I18n.t(100439/*编辑*/),child:<div><i className='foddingicon fooding-alter_icon2'></i>{I18n.t(100439/*编辑*/)}</div>}
               ...
               <div className='col' style={{paddingLeft:0}}>
                    ...
                    <div className='col' style={{paddingLeft:0,paddingTop:0,paddingRight: 0}}>
                            ...
                            otherData={this.props.id}
                             ...
                             columns ={
                                [, {
                                    ...
                                    width: "15%",
                                    render(data, row, index) {
                                        return <i style={{display: 'inline-block', textAlign: 'center'}}
                        ...
                        <MeasureCommon
                            ...
                            onCancel ={this.onCancel} title ={I18n.t(500427/*禁止停靠港口*/)}
                            Zindex={0}
                            openDialog={this.handleClick}
                            DialogTempalte ={require('./DisHarborDialog').default}
                            showHeader ={true}
                            checkedRowsArray={[]}
                            id={'client-detail-distradruleStatn'}
                            onSaveAdd={this.ondisTradruleStatnSaveAdd}
                            onSaveAndClose={this.ondisTradruleStatnSaveAndClose}
                            AjaxInit={true}
                            API_FOODING={API_FOODING_DS}
                            portname={'/tradruleForbidStatn/getInit'}
                            params={{sourceId:this.state.id}}
                            otherData={this.props.id}
                            columns ={
                                [{
