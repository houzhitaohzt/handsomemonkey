...
import {apiGet, permissionsBtn, apiForm} from "../../../../../services/apiCall";
			...
			*/
	        if(this.props.AjaxInit){
	        	if(this.props.isPost  || false){
		        		apiForm(this.props.API_FOODING,this.props.portname,this.props.params, response => {
			...
			return (
				<div>
				  <RightKey id = {this.state.id} isShowMenu={this.props.isShowMenu} handleClick ={this.handleClick} data ={{id:this.props.id,title:this.props.title,data:tempArray,allData:allData,responseData:this.props.responseData}} array = {menuList.filter( (o)=>o['permissions'] ? permissionsBtn(o['permissions']) : 1 )} style={{zIndex:99}}>
					<div className={this.props.perfixCel?this.props.perfixCel:'template1 template2'}style={{zIndex:this.props.Zindex}}>
						...
						<ul className='row'>
							{
								tempArray.map((value,i)=>{
									return (<li key ={i} className="col-xs-6 col-md-6" style={{padding:'0px',textOverflow:"ellipsis",whiteSpace:"nowrap",overflow:'hidden'}}>
										<span style={{width:'118px'}}>{value.key}</span>
										 <Tooltip
												placement="topLeft"
												mouseEnterDelay={0.2}
											...
											>	
											<b style={{fontWeight:"normal"}}>{value.value?value.value:' '}</b>									
										</Tooltip>
									</li>)
								})
							}
