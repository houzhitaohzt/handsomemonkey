...
export class  ContactDialog extends Component{ 
	constructor(props){
		...
		this.state = {
			...
			checkEmail: this.props.initData.contact ? this.props.initData.contact['linkTyId'] : 80, // 判断是否是 邮件
		}
	}
	...
	linkTypeChange = data => {	
		...
		that.setState({linkTypeId:data,checkEmail: data})
	}
	...
	validate = (rule, value, callback) => {
			...
			if(that.state.linkTypeId == 20 || that.data.linkTyId ==20){
				/*  
					1.如果存在'+'号，必须在第一位 
					2.如果存在 '(' ，必须存在 ')' 
				...
				*/
				...
				if(!isBool) callback([i18n.t(200431/*请输入正确的电话号码*/)]);
				callback([])
			}else if(that.state.linkTypeId == 40|| that.data.linkTyId ==40){
				//表示 是QQ
				let isBool = that.validateReg.isQQ.test(value);
				if(!isBool) callback(['QQ'])
				callback([]);
			}else if(that.state.linkTypeId == 80|| that.data.linkTyId ==80){
				...
				callback([]);				
			}else if(that.state.linkTypeId == 100|| that.data.linkTyId ==100){
				//表示是手机
				let initReg = /\d/g;
				let regCNPhone = /^1(3|4|5|7|8)\d{9}$/g;
				let reg = /^\s*\+?\s*(\(\s*\d+\s*\)|\d+)(\s*-?\s*(\(\s*\d+\s*\)|\s*\d+\s*))*\s*$/g;				
				let isBool = reg.test(value) 
				if(!isBool) callback([i18n.t(200433/*请输入正确的手机号码*/)]);
				callback([])	
			}else{
				callback([])
			}
	...
	render(){
		let that = this;
		let {linkTypeId,checkEmail} = this.state;
		const { getFieldProps, getFieldError, getFieldValue} = this.props.form;
		let {id,data,initData} = this.props;
		...
		if(data.number == 0 || data.number == 1){
			...
			<div className={'girdlayout'}>
				<div className={'row'}>
					<div className="form-group col-md-1" style={{textAlign:'right',marginLeft:"140px"}}>
						<Checkbox
				...
				{ ((id=="client-contact-contact") && (checkEmail==80)) ?
					<div className={'row'}>	
						<div className="form-group col-md-12">
							<label className="col-md-offset-2 col-md-0"></label>
							<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
							<span>{i18n.t(300014/*是否接受自动报价*/)}</span>
							<span>&nbsp;&nbsp;&nbsp;</span>
								<Checkbox
									{...getFieldProps('isOfferRec',{
										initialValue:contact ? contact['isOfferRec'] : false
									})}
