...
import jOrgChart from './jquery.jOrgChart';
...
import {ContextMenu, ContextMenuTrigger, MenuItem, SubMenu} from '../../../../components/RightKey';

export class ClientOrganizational extends Component {
    constructor(props) {
        super(props);
    ...
    handClick = (data, type, e) => {
        ...
        if (data == i18n.t(100437/*删除*/)) {
            ...
            let content = require('./OrganizationalEditDialog').default;
            ...
            let content = require('./OrganizationalDialog').default;
            let ele = React.createElement(content, {
                onCancel: this.onCancel,
                onSaveAndClose: this.onSaveAndClose,
                menuData: rightObj,
    ...
    getChild = () => {
        ...
        apiGet(API_FOODING_DS, '/enterpriseOrg/getTreeById', {
            ...
            that.setState({treeData: response.data}, () => {
                ...
                showlist.attr('id', 'org' + this.state.id);
                this.showall(response.data, showlist);
                $("#jOrgChart" + this.state.id).append(showlist);
                $("#org" + this.state.id).jOrgChart({
                    chartElement: "#jOrgChart" + this.state.id,//指定在某个dom生成jorgchart
    ...
    //获取右键时候的高度和宽度
    getXY = (obj, type) => {
        if (type === "onclick") {//点击事件
            if(obj.data.orgCompanyId == this.state.id) return false;
            let {navAddTab} = this.props;
            let name = i18n.t(100311/*客户*/) + `(${obj.data.name})`;
            navAddTab({id: 3, name: name, component: name, url: '/client/detail/' + obj.data.orgCompanyId});
            this.props.router.push({
                pathname: '/client/detail/' + obj.data.orgCompanyId,
                query: {id: obj.data.orgCompanyId, index: 'detail'},
                state: {refresh: true}
            });
            ...
            this.setState({
                ...
                document.getElementById('clientRightKeyOrg' + this.state.id).focus();
            })
        }
    };
...
export default NavConnect(ClientOrganizational);
