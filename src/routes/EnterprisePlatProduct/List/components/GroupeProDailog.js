...
export class GroupeDialog extends Component{
	constructor(props) {
		super(props);
		this.state ={
		    gData: [],
    ...
    loadTreeChild (treeId){
			apiGet(API_FOODING_DS,'/dataMulDiv2/getTree',{id: treeId},(response)=>{
    ...
    onTreeSelect = (ids, {node}) =>{
			this.setState({selectedLable:node.props.title})
	...
	//保存并关闭
	onSaveAndClose(){
		this.props.onSaveAndClose(this.state.checkedKeys[0],this.state.selectedLable)
	}
	onCancel(){
		this.props.onCancel(undefined,undefined);
	}
	componentDidMount(){
		this.getTree();
	}
...
const GroupeForm =createForm()(GroupeDialog);
