...
class CommonForm extends Component{
	...
	lirunNum(e,dat){
		//利润率 利润额 计算成本单价
		let data = dat ||{};
		let salTaxPrc = data.salTaxPrc || this.props.form.getFieldValue("salTaxPrc");//含税单价
		let profTyp = data.profTyp || this.props.form.getFieldValue('profTyp').profTyp;
		if(profTyp == 10){
			let num = data.profRate || (e && e.target && e.target.value)||0;
			if(salTaxPrc && num ){
				let a = parseFloat(salTaxPrc)/(1+parseFloat(num)/100);
				this.props.form.setFieldsValue({
					costPrc:a.toFixed(6)
				})
			}
		}else {
			let profRate = data.ehProf || e.target.value;//利润额
        ...
        apiGet(API_FOODING_ERP, '/saleorder/mtl/getOne', {id: billDtlId},
            response => {
                this.setState({businessOne: response.data, productData: response.data},()=>{
                	if(!response.data.costPrc && response.data.salTaxPrc &&(response.data.profRate || response.data.ehProf)){
						that.lirunNum(null,response.data);
					}
				});
            }, error => {
	...
	render(){
        ...
        let validate = (rule, value, callback) => {
            if(parseFloat(value) > 0){
                callback([]);
            } else callback([i18n.t(400102/*数量不能大于*/)]);
        };
		let content = <div></div>
		//订单产品
