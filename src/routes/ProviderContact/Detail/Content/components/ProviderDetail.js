...
export class ProviderDetail extends Component{
        ...
        if(data.number ==2){
            ...
            if(!(data && data.record || data.selectArr.length == 1)){
             ...
             Confirm(tempString, {
                     ...
                     if(data.id == "provider-Contact-addressList"){//地址列表
                    ...
                    }else if(data.id == "provider-Contact-contact"){//联系方式
    ...
    onContactSaveAndClose(value,data){
        ...
        value = Object.assign({},{beId:this.state.id,dataTyId:120},value);
        if(JSON.stringify(data) !== "{}"){//表明是编辑
            value = Object.assign({},value,{id:data.id,optlock:data.optlock});
        }
        apiPost(API_FOODING_DS,'/contact/save',value,(response) => {
                   ...
                   height:this.state.scrollHeight,overflow:'auto'}}>
                       <div className = 'col'>
                        <div style={{marginBottom:10}}>
                            <Template1
                            menuList={[
                                {permissions:'providercontact.edit',type:i18n.t(100439/*编辑*/),child:<div><i className='foddingicon fooding-alter_icon2'></i>{i18n.t(100439/*编辑*/)}</div>}
                            ]}
                            ...
                            params={{id:this.state.id,sourceId:this.state.sourceId,dataTyId:120}}
                            id={'provider-Contact-Noraml'} title={i18n.t(100138/*常规*/)} tempArray={[
                      ...
                      <MeasureCommon
                        menuList={[
                            {type:'add',permissions:'providercontact.dtl.add'},
                            {type:'delete',permissions:'providercontact.dtl.del'},
                            {type:'edit',permissions:'providercontact.edit'}
                        ]}
                        ...
                        onSaveAndClose={this.onAddressListSaveAndClose}
                         ...
                         showHeader ={false}
                         checkedRowsArray={[]}
                         id={'provider-Contact-addressList'}
                         AjaxInit={true}
                         API_FOODING={API_FOODING_DS}
                         portname={'/address/getInit'}
                         params={{beId:this.state.id,dataTyId:120}}
                         otherData={{beId:this.state.id,dataTyId:120}}
                         columns ={
                            [{
