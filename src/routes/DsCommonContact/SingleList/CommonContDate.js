...
class CommonContDate extends Component{
	...
	addClick = () => {
		let that = this,title;
		let {navAddTab, navRemoveTab,activityType} = that.props; 
	     navRemoveTab({name:I18n.t(100439/*编辑*/) + I18n.t(100588/*联络*/),component:I18n.t(100439/*编辑*/) + I18n.t(100588/*联络*/),url:'/commoncontact/edit'});
	     navAddTab({ name: I18n.t(100392/*新增*/) + I18n.t(100588/*联络*/), component: I18n.t(100392/*新增*/) + I18n.t(100588/*联络*/), url: '/commoncontact/edit'});
	     that.props.router.push({pathname:'/commoncontact/edit',query:{salBeId:this.props.salBeId,originalId:this.props.originalId,originalType:this.props.originalType,activityType:this.props.activityType,salBeLC:encodeURIComponent(this.props.salBeLcName),salBeEN:encodeURIComponent(this.props.salBeEnName),typeNumber:this.props.typeNumber,isDt:this.props.isDt}});
    }
    onRowDoubleClick = (record,index,checked) => {
		let {navRemoveTab,navAddTab,activityType} =this.props;
		navRemoveTab({name:I18n.t(100588/*联络*/) + I18n.t(100097/*详情*/),component:I18n.t(100588/*联络*/) + I18n.t(100097/*详情*/),url:'/commoncontact/detail'});
	    navAddTab({ name:I18n.t(100588/*联络*/) + I18n.t(100097/*详情*/), component:I18n.t(100588/*联络*/) + I18n.t(100097/*详情*/), url: '/commoncontact/detail'});
        this.props.router.push({pathname: '/commoncontact/detail', query: {scheduleId: record.id,salBeId:this.props.salBeId,originalId:this.props.originalId,originalType:this.props.originalType,activityType:this.props.activityType,salBeLC:encodeURIComponent(this.props.salBeLcName),salBeEN:encodeURIComponent(this.props.salBeEnName),typeNumber:this.props.typeNumber,isDt:this.props.isDt}});
	}
	//删除数据
    deleteClick = () => {
    	let that = this;
    	let num = that.refs['commoncontact'+ this.props.type + this.props.activityType].getSelectArr();
    	let tempString = "", id = [];
    	if(!num.length){
	...
	componentWillUnmount() {
  	...
  	componentWillReceiveProps(nextProps){
  		this.handleResize(0);
		window.addEventListener('resize', this.handleResize(0));
		if(this.props.activityType !== nextProps.activityType || this.props.salBeId !== nextProps.salBeId || this.props.typeNumber !== nextProps.typeNumber && !this.props.isDetail){
	...
	render(){
		...
		switch(pageIdent){
			...
			break;
			case 'clientLinkman' :	 // 客户联系人-联络
				var iconArray =  [{type:'add',onClick:this.addClick,permissions:'clientcontact.dtl.add'},{type:'delete',onClick:this.deleteClick,permissions:'clientcontact.dtl.del'}];
			break;
			case 'provider' :	 // 供应商
			...
			case 'providerLinkman' :	 // 供应商联系人
				var iconArray =  [{type:'add',onClick:this.addClick,permissions:'providercontact.dtl.add'},{type:'delete',onClick:this.deleteClick,permissions:'providercontact.dtl.del'}];
			break;
			case 'forwarder' :  // 货代公司 
			...
			case 'forwarderLinkman' :  // 货代公司联系人 
				var iconArray = [{type:'add',onClick:this.addClick,permissions:'forwardercontact.dtl.add'},{type:'delete',onClick:this.deleteClick,permissions:'forwardercontact.dtl.del'}];
			break; 
			case 'server' :  // 服务机构 
			...
			break;
			case 'serverLinkman' :  // 服务机构联系人 
				var iconArray = [{type:'add',onClick:this.addClick,permissions:'servbecontact.dtl.add'},{type:'delete',onClick:this.deleteClick,permissions:'servbecontact.dtl.del'}];
			break;																					
			default:
			...
			<div className="contact-fluid">
				...
				<div className="contact-body" style = {{height:this.state.scrollHeight}}>
					...
					<div className="action-contact-buttons">
						<Table
							ref={'commoncontact'+ this.props.type + this.props.activityType}
