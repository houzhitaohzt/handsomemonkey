...
export class ActivityAddEdit extends Component {
    ...
    addBeforeSaveClick = callback => {
        if (this.state.inputValue == "") {
            ServiceTips({text: I18n.t(500210/*请选择联络目的!*/), type: 'error'});
            return false;
        }
        ...
        form.validateFields((error, value) => {
            if (error) {
                ...
                apiPost(API_FOODING_OA, '/activity/save', param, response => {
                    ...
                    this.props.router.push({pathname: '/commoncontact/edit', query: newQuery, state: {refresh: false}});
                    this.setState({commonData: param})
                    callback(null, param.id);
                }, error => {
                    callback(I18n.t(100498/*保存失败，请稍后再试！*/))
                })
            }
        })
    }
            ...
            if (error) {
                ...
                apiPost(API_FOODING_OA, "/activity/save", valueone, response => {
                    ...
                    navReplaceTab({
                        ...
                        url: '/commoncontact/detail'
                    });
                    that.props.router.push({
                        pathname: '/commoncontact/detail',
                        query: newQuery,
                        state: {refresh: true}
                    });
                }, error => ServiceTips({text: error.message, type: 'error'}))
