...
//引入组织
import ActivityOrigization from "./ActivityOrigization";
...
export class ActivityAddEdit extends Component {
    ...
    //约会目的的点击事件
    onTableClick = (data, originalId) => {
        this.setState({inputValue: data, originalId: originalId || ""});
    }
    ...
    getData = () => {
        ...
        apiGet(API_FOODING_OA, '/activity/getOne', {id: scheduleId}, (response) => {
            ...
            commonData = Object.assign({}, newObject);
            that.setState({
                commonData,
    ...
    addBeforeSaveClick = callback => {
        if (!this.state.inputValue) {
            ServiceTips({text: "" + I18n.t(500209/*请选择响应目的!*/), type: 'error'});
            return false;
        }
        ...
        form.validateFields((error, value) => {
            if (error) {
                ...
                /* param = Object.assign({},param,{
                   datingPurpose:this.state.inputValue,
                   customerParticipantIds:param.customerParticipantIds.map(e => e.customerParticipantIds).toString(),
                   customerParticipantLcNames:param.customerParticipantIds.map(e => e.customerParticipantLcNames).toString(),
                   ...
                   activityType:this.props.location.query.activityType || "",originalId:this.state.originalId
               })*/
                param = Object.assign({}, param, {
                    ...
                    sourceId: this.state.originalId
                })
                apiPost(API_FOODING_OA, '/activity/save', param, response => {
                    ...
                    this.props.router.push({pathname: '/activity/edit', query: newQuery, state: {refresh: false}});
        ...
        form.validateFields((error, value) => {
            if (error) {
                ...
                apiPost(API_FOODING_OA, "/activity/save", valueone, response => {
                    ...
                    that.props.router.push({pathname: '/activity/detail', query: newQuery, state: {refresh: true}});
                }, error => ServiceTips({text: error.message, type: 'error'}))
            }
        })
    }
    ...
    render() {
        ...
        return (
            <div className='addproduct scroll' style={{height: this.state.scrollHeight, overflow: scroll}}>
                ...
                <ActivityOrigization form={form} commonData={this.state.commonData}/>
