...
export class ActivityAddEdit extends Component {
    ...
    addBeforeSaveClick = callback => {
        ...
        form.validateFields((error, value) => {
            if (error) {
                // callback(I18n.t(100142/*否*/))
            } else {
                let param = this.props.form.getFieldsValue();
                param = Object.assign({}, param, {
                    sourceId: this.props.location.query.originalId,
                ...
                apiPost(API_FOODING_OA, '/activity/save', param, response => {
                    ...
                    this.props.router.push({pathname: '/ccontacterp/edit', query: newQuery, state: {refresh: false}});
    ...
    saveClick = () => {
        ...
        form.validateFields((error, value) => {
            if (error) {
                ...
                // if(this.state.businessMtlList == 0){
                ...
                apiPost(API_FOODING_OA, "/activity/save", valueone, response => {
                    ...
                    navReplaceTab({
                        ...
                        component: I18n.t(100588/*联络*/) + I18n.t(100097/*详情*/),
                        url: '/ccontacterp/detail'
                    });
                    that.props.router.push({pathname: '/ccontacterp/detail', query: newQuery, state: {refresh: true}});
                }, error => ServiceTips({text: error.message, type: 'error'}))
            }
        })
    }
                                ...
                                columns={
                                    [{
                                        ...
                                        render(data, row, index) {
                                            if (data == 10) {
                                                return (
                                                    <div>{row.profTypeName}&nbsp;&nbsp;&nbsp;&nbsp;{row.profRate + "%"}</div>);
                                            } else if (data == 20) {
                                                return (
