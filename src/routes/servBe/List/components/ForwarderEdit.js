...
class CommonForm extends Component{
	...
	render(){
		...
		const {countrys,serbBe} = data;
		const { getFieldProps, getFieldError, getNFieldProps } = this.props.form;
		const disabled = form.isFieldsValidating() || form.isSubmitting();
		let aaa = getNFieldProps('id',{
								initialValue:serbBe&&('id' in serbBe)?serbBe.id:''
							});
		let bbb = getNFieldProps('optlock',{
								initialValue:serbBe&&('optlock' in serbBe)?serbBe.optlock:''
							});
		...
		let dom = ( <div className={'addnormal'} style={{marginBottom:'10px'}}>
						...
						<div className={'row'}>
							...
							<label className={'col-md-3 col-lg-3'}>{I18n.t(100339/*服务机构代码*/)}</label>
							<input type="text"  className="text-input-nowidth col-md-9 col-lg-9"  
								placeholder={i18n.t(100378/*自动生成*/)}
								disabled
								{...getNFieldProps('code',{
									validateFirst: true,
									initialValue:serbBe&&('code' in serbBe)?serbBe.code+'':''
								})}
							/>
						</div>
						<div className="form-group col-md-6 col-lg-6">
							...
							<Select
								...
								{...getNFieldProps('cntryId',{
									validateFirst: true,
									rules: [{required:true,}],
									initialValue:serbBe&&serbBe.country?{s_label:serbBe.country.localName,cntryId:serbBe.country.id}:undefined
								})}
					...
					<div className="row">
						<div className="form-group col-md-6 col-lg-6">
							...
							<NameCheck
								...
								fieldName='name'
								rules={true}
								initialValue={serbBe&&('name' in serbBe)?serbBe.name+'':''}
						...
						<div className="form-group col-md-6 col-lg-6">
							...
							<NameCheck
								...
								rules={true}
								isEnName={true}
								initialValue={serbBe&&('enName' in serbBe)?serbBe.enName+'':''}
								className={'col-md-9 col-lg-9'}
							/>
						</div>
					</div>
						...
						<div className="form-group col-md-6 col-lg-6">
							...
							<input type="text" placeholder={""} className="col-md-9 col-lg-9 text-input-nowidth"
								{...getNFieldProps('description',{
									initialValue:serbBe&&('description' in serbBe)?serbBe.description:''
								})}
							/>
						</div>
					</div>
