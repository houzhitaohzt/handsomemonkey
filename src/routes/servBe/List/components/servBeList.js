...
class ServBelist extends Component{
	constructor(props){
		...
		this.columns = [{
			...
			dataIndex : 'entContact',
			key : "entContact",
			width : "12%",
			render(data,row ,index){
				if( !data ) return '';
				return <Card type="servbeLinkMan" data={row['entContact']} router={that.props.router}/>;
			}
		},{
	...
	deleteClick(){
		let numArr = this.refs.servbe.getSelectArr();
		let tempString = "";
		let id = [];
		if(numArr.length==0){
		...
		Confirm(tempString, {
			  	...
			  	apiForm(API_FOODING_DS,'/servBe/delete',{id:id},(response)=>{
	...
	onSaveAndClose(value,data){
		...
		apiPost(API_FOODING_DS,'/servBe/save',value,(response)=>{
			ServiceTips({text:response.message,type:'sucess'});
	...
	onRowDoubleClick(record,index,checked){
		...
		navAddTab({name:I18n.t(100313/*服务机构*/) + I18n.t(100097/*详情*/),component:I18n.t(100313/*服务机构*/) + I18n.t(100097/*详情*/),url:'/servbe/detail'});
		this.props.router.push({pathname:'/servbe/detail',query:{id:record.id,index:'detail'},state: {refresh: true}});
	}
	handleClick(e,data){
		if(data.type == 0){
			apiGet(API_FOODING_DS,'/servBe/getInit',{id:data.record.id},(response)=>{
			...
			Confirm(I18n.t(100435/*是否对该条数据失效？*/), {
			  		...
			  		//表示是失效
					apiForm(API_FOODING_DS,'/servBe/disable',{id:data.record.id},(response)=>{
						ServiceTips({text:response.message,type:'sucess'})
						this.getPages();
					},(error)=>{
			...
			//表示是激活
			apiForm(API_FOODING_DS,'/servBe/enable',{id:data.record.id},(response)=>{
				ServiceTips({text:response.message,type:'sucess'});
				this.getPages();
			},(error) => {
				ServiceTips({text:error.message,type:'error'})
	...
	getPages(currentPage,size,filter,order){
		...
		apiGet(API_FOODING_DS,'/servBe/getPage',params,(response)=>{
	...
	render(){
		...
		return(<div>
			...
			<div ref="servBe" className={'client-body'} style={{height:this.state.scrollHeight}}>
