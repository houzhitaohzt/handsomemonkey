...
class Port extends Component{
	constructor(props){
		...
		this.columns = [{
			...
			title : I18n.t(100160/*港口限重*/),
			dataIndex : "stanLimtWt",
			key : "stanLimtWt",
			width : "10%",
			render(data,row,index){
				return data;
			...
			title : I18n.t(100161/*强制托盘*/),
			dataIndex : "foSalvMark",
			key : "foSalvMark",
			width : "10%",
			render(data,row,index){
	...
	addClick(){
		this.setState({
				...
				showSaveAdd:true,
				showSaveClose:true,
				title: I18n.t(100157/*港口新增*/),
		})
	}
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/statn/getOne',{id:record.record.id},(response)=>{
			this.setState({
	...
	deleteClick(data){
		...
		if(numArr.length > 0){
			...
			Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/statn/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		var that = this;
		let a = [];
		value.transRoutIds?value.transRoutIds.map((e)=>{a.push(e.id)}):'';
		delete value.transRoutIds;

		value=Object.assign({},data,value,{transRoutIds:a});
		apiPost(API_FOODING_DS,'/statn/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	onRowDoubleClick(record,index,checked){
		var that = this;
		apiGet(API_FOODING_DS,'/statn/getOne',{id:record.id},(response)=>{
			that.setState({
				checkedData:response.data,
				rodalShow : true,
				showHeader:true,
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/statn/getPage',object,
				(response)=>{	
					that.setState({	
				...
				<div className="action-buttons">
					<div className={'key-page'}>
						...
						<Dialog width={926}  visible={this.state.rodalShow} title={this.state.title} showHeader ={this.state.showHeader}>
							<PortPlug DialogContent={this.state.DialogContent}
...
export default NavConnect(Port);
