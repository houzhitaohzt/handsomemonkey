...
class TaxType extends Component{
	...
	editClick(record){
		var that = this;
		apiGet(API_FOODING_DS,'/taxSubType/getOne',{id:record.record.id},(response)=>{
			that.setState({
				rodalShow:true,
				showHeader:true,
				DialogContent:3,
	...
	deleteClick(data){
				...
				if(numArr.length > 0){
					...
					Confirm(I18n.t(100433/*删除后将无法恢复，您确定删除吗？*/), {
						    ...
						    apiForm(API_FOODING_DS,'/taxSubType/delete',{id:value},(response)=>{
						    	that.getPage();
	...
	onSaveAndClose(value,data,isAdd){
		...
		apiPost(API_FOODING_DS,'/taxSubType/save',value,(response)=>{
				that.setState({
					rodalShow:!!isAdd
				})
				 ServiceTips({text:response.message,type:'success'});
	...
	getPage(currentPage,objValue){
		...
		apiGet(API_FOODING_DS,'/taxSubType/getPage',object,
				(response)=>{	
					that.setState({	
