...
function filterFn(input, child) {
    return String(getPropValue(child, this.props.optionFilterProp)).toUpperCase().indexOf(input.toUpperCase()) > -1;
}
...
const Select = React.createClass({
    ...
    onMenuSelect({item}) {
        ...
        const selectObjValue = item.props.objValue;
    ...
    getVLForOnChange(vls_) {
        ...
        if (vls !== undefined) {
            if (!this.props.labelInValue) {
                vls = vls.map(v => v.objValue || v.key);
            } else {
    ...
    render() {
        ...
        ><i className={'foddingicon fooding-menu_delete_32'}></i></span>);
        return (
            <SelectTrigger
