...
const DialogWrapper = props => {
    ...
    const className = `fooding-dialog-dialog fooding-dialog-${props.animation}-${props.animationType}`;
    ...
    return (
        <div className={'dialog-wrapper'} onClick={(e)=>{
            e.stopPropagation();
            // e.preventDefault();
        }}>
            <div className={'content'}>
                <div style={mergedStyles} className={className}>
                        {header}
                    <div className={'dialog-content'} style={props.isShowPicture?{padding:0}:{}}>
                        {props.children}
                        {
                            props.showConfirmClose?<i className={'foddingicon fooding-close picture-close'} onClick={props.onClose}></i>:null
                        }
                    </div>
                    {footer}
                </div>
...
class Dialog extends Component {
    ...
    render() {
        ...
        const mask = this.props.showMask ? <div className="fooding-dialog-mask" style={this.props.customMaskStyles} onClick={this.props.onClose} >
