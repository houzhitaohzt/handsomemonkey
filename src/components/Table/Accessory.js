...
export class ColorColumn extends Component {
    constructor(props) {
        ...
        if (defaultValue == null) {
            defaultValue = 'foddingicon fooding-lianjie';
        }
    ...
    static defaultProps = {
        ...
        rowData: {},
        colors: [
            {color: 'foddingicon fooding-lianjie yes', className: 'foddingicon fooding-accessory-yes-new'},
            {color: 'foddingicon fooding-lianjie no', className: 'foddingicon fooding-accessory-no-new h'},
            {color: 'foddingicon fooding-lianjie all', className: 'foddingicon fooding-cross'}
        ]
    };
                ...
                <Menu onSelect={this.emailSelect} style={{marginBottom:0}}>
                    {
                        this.props.colors.map((item, index) => (
                          <MenuItem className={item.className} style={{fontSize:'16px',textAlign:'center'}} key={item.color}></MenuItem>
                        ))
                    }
                </Menu>
            }
