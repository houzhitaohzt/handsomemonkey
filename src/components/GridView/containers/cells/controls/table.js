 ...
 * 左のセルラインを描画
 */
function drawBorderLeft(canvas, sheet, topLeft, cellPoint, rect, cell) {
...
function drawBorderTop(canvas, sheet, topLeft, cellPoint, rect, cell) {
...
// セル枠の描画
function drawBorder(canvas, sheet, topLeft, cellPoint, rect, cell) {
 ...
 * 対象セルが描画可能か
 */
function CanCellView(cell, sheet, topLeft, cellPoint) {
    if (!cell.mergeRange) {
        return true;
    }
 ...
 * セルの描画
 */
function drawCell(canvas, sheet, topLeft, cellPoint, cellRect) {
...
// 行内の列描画
function drawColumn(canvas, sheet,  rowNo, rowHeaderItem, topLeft,  cellPoint, cellRect) {
