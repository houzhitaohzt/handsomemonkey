...
export default  class extends Component{
	...
	onRowClick(row,e){
		...
		let {valueKeys, fieldName} = this.props;
		valueKeys = valueKeys || (da => ({
            [fieldName]: da.id,
            s_label: da.localName || da.name
        }));
		this.props.form.setFieldsValue({[fieldName]: valueKeys(row)});
	...
	_onKeyUpEvent = event =>{
  	...
  	render(){
        ...
        let {fieldName, initialValue, rules, style= {}} = this.props;
        let field = getNFieldProps(fieldName, {
            validateFirst: true,
            rules: [this.getValidateRules(this.props)],
            initialValue: initialValue
        });
  		return <div style={{padding: 0, ...style}}
            className={getFieldError(fieldName)?' error-border product-select ' + this.props.className:'product-select ' + this.props.className}>
            <input type="text" className='text-input-nowidth' placeholder=""
                   ...
                   style={{width:"100%"}}
            />
			<i className={field.value && !rules ?'foddingicon fooding-clear yashi clear':'none'}
            ...
            <div className={this.state.visbled?"dialog":"none"} tabIndex="-1" id={'dialog'} style={{width: 550, position: 'absolute'}}>
