...
const Lunar = {
    ...
    //@param year 阳历年
    //@param between 天数
    lunarByBetween (year, between, month, day) {
        ...
        //return lunarArray;
        return this.festival(lunarArray, month, day);
    },
    //根据得出的农历时间, 计算出每天的节日
    //@param lunarArray 得出的农历
    //@param month 公历月
    //@param day 公历天
    festival (lunarArray, month, day) {
        let newlunar = lunarArray, n , g,
        monthHash = ['', '正月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '冬月', '腊月'];
        let index = monthHash.indexOf(lunarArray[5]) != -1? monthHash.indexOf(lunarArray[5]).toString():"";
        let a = index.length == 1?("0" + index):index;
        let b = lunarArray[2].toString().length == 1 ? ("0" + lunarArray[2].toString()):lunarArray[2].toString();
        n = this.lFtv[a + b];
        let c = month.toString().length == 1?("0" + month.toString()):month.toString();
        let d = day.toString().length == 1?("0" + day.toString()):day.toString();
        g = this.sFtv[c + d];
        newlunar.push(n,g);
        return newlunar;
    },
    ...
    //中文日期
    chineseNumber (num) {
        let dateHash = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'], res = "";
        if (num <= 10) {
            res = '初'+ dateHash[num];
    ...
    toLunar (year, month, day) {
        ...
        return this.lunarByBetween(year, this.betweenSolarDays(year, month, day, yearData[1], yearData[2]), month, day);
    },
    //转换公历
    //@param year 阴历-年
