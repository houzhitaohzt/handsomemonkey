...
import ProductHead from '../../routes/Provider/Detail/Header/PVetailsHead'
//引入ajax请求
...
import Detail from  '../../routes/Provider/Detail/Content/components/ProviderDetail';
import Product from  '../../routes/Provider/Product/List/components/Product';
import LinkMan from  '../../routes/Provider/Linkman/components/Linkman';
import Enterprise from  '../../routes/Provider/Enterprise/components/Enterprise';
import Purchaseorder from  '../../routes/Provider/Purchaseorder/components/Purchaseorder';
import Email  from  '../../routes/Provider/Email/components/Email';
import Date from  '../../routes/Provider/Date/components/ProviderDate';
import Contact from  '../../routes/Provider/Contact/components/ProviderContact';
import Activity from  '../../routes/Provider/Activity/components/ProviderActivity';
import Accessory from  '../../routes/Provider/Accessory/components/Accessory';
import Annotation from  '../../routes/Provider/Annotation/components/Annotation';
import Organizational from  '../../routes/Provider/Organizational/components/Organizational';
...
export class ProviderLayout extends Component{
	...
	getDetailData(obj){
		apiGet(API_FOODING_DS,'/vendor/getDetail',{id:this.state.id},(response)=>{
			let {vendor,regCapitalCurren,country} = response.data;
			    ...
			    <div className={'detail-layout__viewport'} style={{paddingTop:this.state.paddingTop}}>
					<div>
						...
						<div className={this.state.curentId == 'organizational' ?"":"none"}>
							<Organizational {...newProps} organizational ={no => this.organizational = no} param={{dataTyId:40}} />
						</div>
						<div className={this.state.curentId == 'customs' ?"":"none"}>
							<Customs {...newProps} customs ={no => this.customs = no} param={{dataTyId:40,vendor:vendor['customsDataTxt']}} isDetail ={true} />
						</div>						
					</div>
			    </div>
			</div>
